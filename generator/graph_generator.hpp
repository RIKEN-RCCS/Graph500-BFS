...
class EdgeListStorage {
 public:
...
#if VERVOSE_MODE
    if (mpi.isMaster()) {
      print_with_prefix("Allocating edge list memory (%" PRId64 " * %d bytes)",
  ...
  int beginRead(bool release_buffer) {
    ...
    if (data_in_file_) {
      ...
      if (edge_memory_ == NULL) {
        read_buffer_ = static_cast<EdgeType*>(
            cache_aligned_xmalloc(CHUNK_SIZE * 2 * sizeof(EdgeType)));
      ...
      if (write_offset_ + count <= read_offset) {
        ...
        if (write_buffer_filled_size_ + count > write_buffer_size_) {
          ...
          while (write_buffer_filled_size_ + count > write_buffer_size_) {
            write_buffer_size_ =
                std::max(INT64_C(4096), write_buffer_size_ * 2);
          }
        ...
        memcpy(write_buffer_ + write_buffer_filled_size_, edge_data,
               count * sizeof(EdgeType));
 ...
 private:
  void getReadBuffer(EdgeType** pp_buffer_to_read,
                     EdgeType** pp_buffer_for_user) {
  ...
  void writeInternal(EdgeType* edge_data, int count) {
    if (edge_memory_ != NULL) {
      if (write_offset_ + count > edge_memory_size_) {
        ...
        EdgeType* new_memory = static_cast<EdgeType*>(
            realloc(edge_memory_, (write_offset_ + count) * sizeof(EdgeType)));
        if (new_memory == NULL) {
...
enum type { NONE, BINARY_TREE, HAMILTONIAN_CYCLE };
}

class GraphGeneratorBase {
 public:
  GraphGeneratorBase(int scale, int edge_factor, int max_weight, int userseed1,
                     int userseed2, InitialEdgeType::type initial_edge_type)
      : scale_(scale),
  ...
  int64_t num_global_edges() const {
    return num_global_verts() * edge_factor_ + num_initial_edges_;
  }

  /* Reverse bits in a number; this should be optimized for performance
   * (including using bit- or byte-reverse intrinsics if your platform has
   * them).
   * */
...
#undef USE_GCC_BYTESWAP
 ...
 protected:
  ...
  void generateWeight(EdgeType* edge_buffer, int64_t start_edge,
                      int64_t end_edge,
...
#pragma omp for
    ...
    const int64_t partition_size =
        ((end_edge - start_edge) + num_threads - 1) / num_threads;
    int64_t thread_start_edge = start_edge + partition_size * thread_num;
    int64_t thread_end_edge =
        std::min<int64_t>(thread_start_edge + partition_size, end_edge);
    mrg_state new_state = mrg_seed();
    mrg_skip(&new_state, 30, 46, thread_start_edge);

    for (int64_t edge_index = thread_start_edge; edge_index < thread_end_edge;
         ++edge_index) {
#if 1  // for debug
      edge_buffer[edge_index - start_edge].weight_ = 0xBEEF;
#else
      edge_buffer[edge_index - start_edge].weight_ =
          (mrg_get_uint_orig(&new_state) % max_weight_) + 1;
#endif
    }
#endif
  }
  ...
  template <typename EdgeType>
  void generateInitialEdge(EdgeType* edge_buffer, int64_t start_edge,
                           int64_t end_edge) const {
    switch (initial_edge_type_) {
      case InitialEdgeType::BINARY_TREE:
#pragma omp for
...
class GraphGenerator : public GraphGeneratorBase {
 public:
  GraphGenerator(int scale, int edge_factor, int max_weight, int userseed1,
                 int userseed2, InitialEdgeType::type initial_edge_type)
      : GraphGeneratorBase(scale, edge_factor, max_weight, userseed1, userseed2,
                           initial_edge_type) {}
  virtual ~GraphGenerator() {}
  virtual void generateRange(EdgeType* edge_buffer, int64_t start_edge,
                             int64_t end_edge) const = 0;
};
...
class RandomGraphGenerator : public GraphGenerator<EdgeType> {
 ...
 public:
  RandomGraphGenerator(int scale, int edge_factor, int max_weight,
                       int userseed1, int userseed2,
                       InitialEdgeType::type initial_edge_type)
      : GraphGenerator<EdgeType>(scale, edge_factor, max_weight, userseed1,
                                 userseed2, initial_edge_type) {}

  virtual void generateRange(EdgeType* edge_buffer, int64_t start_edge,
                             int64_t end_edge) const {
    if (start_edge < this->num_initial_edges()) {
      BaseType::generateInitialEdge(
          edge_buffer, start_edge,
          std::min(end_edge, this->num_initial_edges()));
    }
...
#pragma omp for
    for (int64_t edge_index = std::max(start_edge, this->num_initial_edges());
      ...
      edge_buffer[edge_index - start_edge].set(
          this->scramble(mrg_get_uint_orig(&new_state) &
                         num_global_verts_minus1),
          this->scramble(mrg_get_uint_orig(&new_state) &
                         num_global_verts_minus1));
    }
...
template <typename EdgeType, int INITIATOR_A_NUMERATOR,
          int INITIATOR_BC_NUMERATOR>
class RmatGraphGenerator : public GraphGenerator<EdgeType> {
  typedef GraphGenerator<EdgeType> BaseType;

 public:
  RmatGraphGenerator(int scale, int edge_factor, int max_weight, int userseed1,
                     int userseed2, InitialEdgeType::type initial_edge_type)
      : GraphGenerator<EdgeType>(scale, edge_factor, max_weight, userseed1,
                                 userseed2, initial_edge_type) {}
...
#pragma omp for
    for (int64_t edge_index = std::max(start_edge, this->num_initial_edges());
      ...
      mrg_state new_state = this->mrg_seed();
      mrg_skip(&new_state, 0, edge_index, 0);
      make_one_edge(this->num_global_verts(), 0, &new_state,
...
#undef SPK_NOISE_LEVEL
  ...
  void make_one_edge(int64_t nverts, int level, mrg_state* st,
                     EdgeType* result) const {
