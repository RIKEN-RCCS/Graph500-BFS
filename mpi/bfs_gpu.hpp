...
class BfsOnGPU : public BfsBase<Pack48bit, LocalVertsIndex, BfsOnGPU_Params> {
 ...
 public:
  ...
  virtual ~BfsOnGPU() {
    ...
    // delete device graph
    if (graph_on_gpu_) {
      CUDA_CHECK(cudaFree(this->dev_row_starts_));
      this->dev_row_starts_ = NULL;
      CUDA_CHECK(cudaFree(this->dev_index_array_high_));
      this->dev_index_array_high_ = NULL;
      CUDA_CHECK(cudaFree(this->dev_index_array_low_));
      this->dev_index_array_low_ = NULL;
    }
    CUDA_CHECK(cudaFree(this->dev_invert_vertex_mapping_));
    this->dev_invert_vertex_mapping_ = NULL;
  ...
  void construct(EdgeList* edge_list) {
    ...
    if (graph_on_gpu_) {
      CUDA_CHECK(
          cudaMalloc((void**)&this->dev_row_starts_,
                     sizeof(this->dev_row_starts_[0]) * (num_columns + 2)));
      CUDA_CHECK(
          cudaMalloc((void**)&this->dev_index_array_high_,
                     sizeof(this->dev_index_array_high_[0]) * index_size));
      CUDA_CHECK(
          cudaMalloc((void**)&this->dev_index_array_low_,
    ...
    CUDA_CHECK(cudaMalloc(
        (void**)&this->dev_invert_vertex_mapping_,
        sizeof(this->dev_invert_vertex_mapping_[0]) * num_local_vertices));

    if (graph_on_gpu_) {
      CUDA_CHECK(
          cudaMemcpy(this->dev_row_starts_, this->graph_.row_starts_,
                     sizeof(this->dev_row_starts_[0]) * (num_columns + 1),
                     cudaMemcpyHostToDevice));
      CUDA_CHECK(cudaMemcpy(this->dev_index_array_high_,
                            this->graph_.index_array_.get_ptr_high(),
                            sizeof(this->dev_index_array_high_[0]) * index_size,
                            cudaMemcpyHostToDevice));
      CUDA_CHECK(cudaMemcpy(this->dev_index_array_low_,
                            this->graph_.index_array_.get_ptr_low(),
                            sizeof(this->dev_index_array_low_[0]) * index_size,
                            cudaMemcpyHostToDevice));
      // add an empty column
      CUDA_CHECK(cudaMemcpy(this->dev_row_starts_ + num_columns + 1,
                            &index_size, sizeof(this->dev_row_starts_[0]),
                            cudaMemcpyHostToDevice));
    }
    CUDA_CHECK(cudaMemcpy(
        this->dev_invert_vertex_mapping_, this->graph_.invert_vertex_mapping_,
        sizeof(this->dev_invert_vertex_mapping_[0]) * num_local_vertices,
        cudaMemcpyHostToDevice));
    CudaStreamManager::end_cuda();
  ...
  void prepare_bfs() {
    ...
    if (mpi.isMaster()) {
      ...
      double bfs_temp_buffer_size =
          sizeof(int) * this->get_summary_size_v0() + sizeof(this->dev_[0]) +
          std::max<int64_t>(
              sizeof(this->dev_column_buffer_[0]) * this->get_bitmap_size_v0() +
                  sizeof(cuda::FoldIOBuffer) * NUM_IO_BUFFERS +
                  sizeof(cuda::FoldGpuBuffer),
  ...
  void run_bfs(int64_t root, int64_t* pred) {
    ...
    g_fold_send = g_fold_recv = g_bitmap_send = g_bitmap_recv = g_exs_send =
        g_exs_recv = 0;
...
#define VERTEX_LOCAL(v) ((v) >> log_size)
    ...
    const int64_t sched_threshold =
        this->get_number_of_local_vertices() * mpi.size_2dr / 16;
    const int long_job_length =
        get_blocks(this->get_summary_size_v0(), LONG_JOB_WIDTH);
    const int short_job_length =
        get_blocks(this->get_summary_size_v0(), SHORT_JOB_WIDTH);
    ...
    tmp = MPI_Wtime();
    if (mpi.isMaster())
      fprintf(IMD_OUT, "Time of first expansion: %f ms\n",
              (tmp - prev_time) * 1000.0);
    expand_time += tmp - prev_time;
    prev_time = tmp;
#endif
#undef VERTEX_OWNER
#undef VERTEX_LOCAL
    ...
    int64_t num_columns_per_launch =
        blocks_per_launch_ * GPU_PARAMS::THREADS_PER_BLOCK;
    ...
    while (true) {
      ...
      if (graph_on_gpu_) {
        ...
        num_submit_jobs = std::min<int>(this->d_->num_remaining_extract_jobs_,
                                        num_stream_jobs_);
      } else {
        ...
        this->d_->num_remaining_extract_jobs_ = num_submit_jobs = std::min<int>(
            std::min<int>(number_of_cpu_jobs, omp_get_max_threads() + 5),
            num_stream_jobs_);
      }
#if VERVOSE_MODE
      ...
      if (num_submit_jobs == 0) {
        this->fiber_man_.submit_array(this->sched_.fold_end_job, mpi.size_2dc,
                                      0);
      }
...
#pragma omp parallel
...
#if VERVOSE_MODE
      tmp = MPI_Wtime();
      stall_time += tmp - prev_time;
      prev_time = tmp;
#endif
#if DEBUG_PRINT
      if (mpi.isMaster())
        printf("global_nq_vertices=%" PRId64 "\n", global_nq_vertices);
#endif
      if (global_nq_vertices == 0) break;

      this->expand(global_nq_vertices, &gpu_cq_comm_, &gpu_visited_comm_);
#if VERVOSE_MODE
      tmp = MPI_Wtime();
      if (mpi.isMaster())
        fprintf(IMD_OUT, "Time of levle %d: %f ms\n", this->current_level_,
                (MPI_Wtime() - level_start_time) * 1000.0);
      expand_time += tmp - prev_time;
      prev_time = tmp;
#endif
    }
#if VERVOSE_MODE
    if (mpi.isMaster())
      fprintf(IMD_OUT, "Time of BFS: %f ms\n",
              (MPI_Wtime() - start_time) * 1000.0);
    double time3[3] = {fold_time, expand_time, stall_time};
    double timesum3[3];
    int64_t commd[7] = {g_fold_send, g_fold_recv, g_bitmap_send, g_bitmap_recv,
                        g_exs_send,  g_exs_recv,  cuda_io_bytes_};
    int64_t commdsum[7];
    double kernel_time[6] = {
        kernel_time_create_cq_list_, kernel_time_read_graph_,
        kernel_time_filter_edge_,    kernel_time_receive_process_,
        kernel_time_update_bitmap_,  cuda_time_extract_};
    double kernel_time_sum[6];
    int kernel_count[5] = {
        kernel_launch_create_cq_list_, kernel_launch_read_graph_,
        kernel_launch_receive_process_, kernel_launch_update_bitmap_,
        retry_count_read_graph_};
    int kernel_count_sum[5];
    MPI_Reduce(time3, timesum3, 3, MPI_DOUBLE, MPI_SUM, 0, mpi.comm_2d);
    MPI_Reduce(commd, commdsum, 7, get_mpi_type(commd[0]), MPI_SUM, 0,
               mpi.comm_2d);
    MPI_Reduce(kernel_time, kernel_time_sum, 5, get_mpi_type(kernel_time[0]),
               MPI_SUM, 0, mpi.comm_2d);
    MPI_Reduce(kernel_count, kernel_count_sum, 5, get_mpi_type(kernel_count[0]),
               MPI_SUM, 0, mpi.comm_2d);
    if (mpi.isMaster()) {
  ...
  void end_bfs() { deallocate_memory_gpu(); }
  ...
  struct CuCreateCQList : public CudaCommand {
    ...
    virtual void launch_kernel(cudaStream_t stream) {
      ...
      CUDA_CHECK(cudaMemcpyAsync(&this_->dev_->nq_count,
                                 &this_->host_->nq_count,
                                 sizeof(this_->host_->nq_count),
      ...
      cuda::create_cq_list(this_->dev_cq_bitmap_, this_->dev_cq_summary_,
                           ...
                           this_->get_number_of_edge_columns(), master_stream);
    ...
    virtual void receive(cudaStream_t stream) {
      int* dev_last_offset =
          &this_->dev_cq_count_->get_buffer()[this_->get_summary_size_v0()];
      CUDA_CHECK(cudaMemcpyAsync(&this_->host_->num_non_zero_columns,
                                 dev_last_offset, sizeof(dev_last_offset[0]),
                                 cudaMemcpyDeviceToHost, stream));
#if VERVOSE_MODE
      this_->cuda_io_bytes_ += sizeof(dev_last_offset[0]);
#endif
    };
    virtual void complete() { this_->fiber_man_.end_processing(); };
    ThisType* this_;
  };
  ...
  struct CuExtractEdge : public CudaCommand, public Runnable {
    ...
    virtual bool init() {
      ...
      int64_t num_columns_per_launch =
          this_->blocks_per_launch_ * GPU_PARAMS::THREADS_PER_BLOCK;
      ...
      num_blocks_ = get_blocks<GPU_PARAMS::THREADS_PER_BLOCK>(column_end_ -
                                                              column_start_);
      /*
...
#endif
      dev_buf_ = &this_
                      ->dev_fold_io_buffer_[(this_->fold_io_buf_count_++) %
                                            NUM_IO_BUFFERS]
                      .read_graph;
      // launch kernel
      cuda::read_graph_1(
          &dev_buf_->input, this_->dev_row_starts_,
          this_->dev_index_array_high_, this_->dev_index_array_low_,
          this_->dev_column_buffer_, column_start_, column_end_,
          this_->get_number_of_edge_columns(), this_->dev_shared_visited_,
          this_->dev_fold_proc_buffer_->read_graph.out_columns,
          this_->dev_fold_proc_buffer_->read_graph.out_indices, dev_buf_->edges,
          stream);
#if 1
#if VERVOSE_MODE
    ...
    virtual void receive(cudaStream_t stream) {
      CUDA_CHECK(cudaMemcpyAsync(&host_buf_->read_graph.input, &dev_buf_->input,
                                 sizeof(host_buf_->read_graph.input),
                                 cudaMemcpyDeviceToHost, stream));
      CUDA_CHECK(cudaStreamSynchronize(stream));
#if VERVOSE_MODE
      ...
      CUDA_CHECK(cudaMemcpyAsync(
          &host_buf_->read_graph.edges, &dev_buf_->edges,
          sizeof(host_buf_->read_graph.edges[0]) * output_length,
          cudaMemcpyDeviceToHost, stream));
#if VERVOSE_MODE
      this_->cuda_io_bytes_ +=
          sizeof(host_buf_->read_graph.edges[0]) * output_length;
#endif
      ...
      if (need_retry_) {
        if (output_length == 0) {
          printf(
              "Fatal Error: The length of output buffer"
              " for read graph kernel is too small for this graph.\n");
          sleep(1);
        } else {
#if VERVOSE_MODE
          // printf("Warning: Retrying read graph happens. This degrade
          // performance a lot !\n");
...
#endif
          this_->fiber_man_.submit_array(this_->sched_.fold_end_job,
                                         mpi.size_2dc, 0);
          return;
        }
      }
    ...
    void process_send_edges() {
      // process sending
      typename super_::ThreadLocalBuffer* tl =
          this_->thread_local_buffer_[omp_get_thread_num()];
      bfs_detail::FoldPacket* packet_array = tl->fold_packet;
      const int log_local_verts = this_->graph_.log_local_verts();
      const int64_t v0_high_mask = int64_t(mpi.rank_2dc)
                                   << this_->graph_.log_local_v0();
      const int64_t local_verts_mask =
          this_->get_number_of_local_vertices() - 1;
  ...
  struct CuExtractEdge2 : public CuExtractEdge {
    ...
    virtual void send(cudaStream_t stream) {
      in_dev_buf_ = &this_
                         ...
                         .edge_io;
      CUDA_CHECK(cudaMemcpyAsync(
          in_dev_buf_->columns, this->host_buf_->edge_io.columns,
          sizeof(this->host_buf_->edge_io.columns[0]) * num_read_edges_,
          cudaMemcpyHostToDevice, stream));
      CUDA_CHECK(cudaMemcpyAsync(
          in_dev_buf_->indices, this->host_buf_->edge_io.indices,
          sizeof(this->host_buf_->edge_io.indices[0]) * num_read_edges_,
          cudaMemcpyHostToDevice, stream));
#if VERVOSE_MODE
      this_->cuda_io_bytes_ +=
          sizeof(this->host_buf_->edge_io.columns[0]) * num_read_edges_;
      ...
      this->dev_buf_ =
          &this_
               ->dev_fold_io_buffer_[(this_->fold_io_buf_count_++) %
                                     NUM_IO_BUFFERS]
               .read_graph;
      cuda::filter_edges(&this->dev_buf_->input, num_read_edges_,
                         in_dev_buf_->columns, in_dev_buf_->indices,
                         this_->dev_shared_visited_, this->dev_buf_->edges,
                         stream);
...
#if DEBUG_PRINT
      if (i_end != 0) {
        printf("L:%d:R(%d):start extract_edge_cpu. i_start=%" PRId64
               ", i_end=%" PRId64 ", ii_progress=%" PRId64
               ", k_progress=%" PRId64 "\n",
               __LINE__, mpi.rank_2d, i_progress, i_end, ii_progress,
               k_progress);
      }
#endif
      ...
      for (int64_t i = i_progress; i < i_end; ++i) {
        ...
        for (int ii = ii_progress; ii < (int)sizeof(cq_summary[0]) * 8; ++ii) {
          if (summary_i & (BitmapType(1) << ii)) {
            int64_t cq_base_offset =
                ((int64_t)i * sizeof(cq_summary[0]) * 8 + ii) *
                super_::NUMBER_CQ_SUMMARIZING;
            for (int k = k_progress; k < super_::NUMBER_CQ_SUMMARIZING; ++k) {
              ...
              if (edge_list_length + num_read_edges >
                  GPU_PARAMS::READ_GRAPH_OUTBUF_SIZE) {
                if (num_read_edges == 0) {
                  printf(
                      "Fatal Error: The length of output buffer"
...
#if DEBUG_PRINT
                printf("L:%d:R(%d):launch kernel. i_progress=%" PRId64
                       ", ii_progress=%" PRId64 ", k_progress=%" PRId64 "\n",
                       __LINE__, mpi.rank_2d, i_progress, ii_progress,
                       k_progress);
#endif
                this_->cuda_man_->submit(this, 0);
                return false;
              ...
              for (int64_t ri = row_starts[e0]; ri < row_starts[e0 + 1]; ++ri) {
                int8_t row_lowbits =
                    (int8_t)(index_array.low_bits(ri) %
                             super_::NUMBER_PACKING_EDGE_LISTS);
    ...
    bool get_new_job() {
      int64_t summary_start = __sync_fetch_and_add(
          &this_->current_column_index_, this_->columns_per_task_);
      int64_t summary_size = this_->get_summary_size_v0();
      if (summary_start >= summary_size) {
        return false;
      }
      this->column_start_ = i_progress = summary_start;
      this->column_end_ = std::min<int64_t>(
          summary_size, this->column_start_ + this_->columns_per_task_);
      ii_progress = 0;
      k_progress = 0;
      return true;
    }
  ...
  struct CuReceiveProcessing : public CudaCommand {
    CuReceiveProcessing(ThisType* this__) : this_(this__) {
      CUDA_CHECK(
          cudaMallocHost((void**)&host_packet_list_,
                         sizeof(int4) * GPU_PARAMS::GPU_BLOCK_MAX_PACKTES));
    }
    ~CuReceiveProcessing() { CUDA_CHECK(cudaFreeHost(host_packet_list_)); }
    void set(std::vector<bfs_detail::FoldCommBuffer*>& data, bool last_task) {
      ...
      for (int i = 0; i < (int)recv_data_.size() - 1; ++i) {
        ...
        assert(recv_data_[i + 1].v0_offset -
                       (int)sizeof(bfs_detail::PacketIndex) <
                   v0_offset &&
    ...
    virtual void launch_kernel(cudaStream_t stream) {
      ...
      cuda::decode_varint_stream_signed(dev_buf_->packet_list,
                                        dev_buf_->v0_stream, num_packets_,
                                        dev_v0_list, stream);
#if DEBUG_PRINT
      // TODO: short cut if 0
      ...
      if (num_edges > 0) {
        CUDA_CHECK(
            cudaMallocHost((void**)&host_v0_list, num_edges * sizeof(int64_t)));
        CUDA_CHECK(cudaThreadSynchronize());
        CUDA_CHECK(cudaMemcpy(host_v0_list, dev_v0_list,
                              num_edges * sizeof(int64_t),
                              cudaMemcpyDeviceToHost));
    ...
    virtual void receive(cudaStream_t stream) {
      if (last_task_) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(&this_->host_->nq_count,
                                   &this_->dev_->nq_count, sizeof(int),
                                   cudaMemcpyDeviceToHost, stream));
        int64_t transfer_size =
            sizeof(this_->nq_bitmap_[0]) * this_->get_bitmap_size_visited();
        CUDA_CHECK(cudaMemcpyAsync(this_->nq_bitmap_, this_->dev_nq_bitmap_,
                                   transfer_size, cudaMemcpyDeviceToHost,
                                   stream));
        CUDA_CHECK(cudaMemcpyAsync(this_->nq_sorted_bitmap_,
                                   this_->dev_nq_sorted_bitmap_, transfer_size,
                                   cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(this_->visited_, this_->dev_visited_,
                                   transfer_size, cudaMemcpyDeviceToHost,
                                   stream));
#if VERVOSE_MODE
    ...
    virtual void complete() {
      ...
      if (last_task_) {
        ...
        cuda::cu_clear_nq(this_->get_bitmap_size_visited(),
                          this_->dev_nq_bitmap_, this_->dev_nq_sorted_bitmap_,
                          this_->cuda_stream_array_, NUM_CUDA_STREAMS);
      }
  ...
  int64_t get_number_of_edge_columns() {
    return int64_t(1) << (super_::graph_.log_local_v0() -
                          super_::LOG_PACKING_EDGE_LISTS);
  }
  ...
  void allocate_memory_gpu() {
    ...
    CudaStreamManager::begin_cuda();
    this->nq_bitmap_ = (BitmapType*)page_aligned_xcalloc(
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited());
    CUDA_CHECK(cudaHostRegister(
        this->nq_bitmap_,
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited(), 0));

    this->nq_sorted_bitmap_ = (BitmapType*)page_aligned_xcalloc(
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited());
    CUDA_CHECK(cudaHostRegister(
        this->nq_sorted_bitmap_,
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited(), 0));

    this->visited_ = (BitmapType*)page_aligned_xcalloc(
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited());
    CUDA_CHECK(cudaHostRegister(
        this->visited_,
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited(), 0));
    ...
    if (graph_on_gpu_) {
      CUDA_CHECK(
          cudaMalloc((void**)&this->dev_cq_bitmap_,
                     sizeof(this->cq_bitmap_[0]) * this->get_bitmap_size_v0()));
      CUDA_CHECK(cudaMalloc(
          (void**)&this->dev_cq_summary_,
          sizeof(this->cq_summary_[0]) * this->get_summary_size_v0()));
    } else {
      this->cq_bitmap_ = (BitmapType*)page_aligned_xcalloc(
          sizeof(this->cq_bitmap_[0]) * this->get_bitmap_size_v0());
      this->cq_summary_ = (BitmapType*)malloc(sizeof(this->cq_summary_[0]) *
                                              this->get_summary_size_v0());
      this->dev_cq_bitmap_ = NULL;
      this->dev_cq_summary_ = NULL;
    }
    CUDA_CHECK(cudaMalloc(
        (void**)&this->dev_shared_visited_,
        sizeof(this->shared_visited_[0]) * this->get_bitmap_size_v1()));
    CUDA_CHECK(cudaMalloc(
        (void**)&this->dev_nq_bitmap_,
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited()));
    CUDA_CHECK(cudaMalloc(
        (void**)&this->dev_nq_sorted_bitmap_,
        sizeof(this->nq_bitmap_[0]) * this->get_bitmap_size_visited()));
    CUDA_CHECK(cudaMalloc(
        (void**)&this->dev_visited_,
        sizeof(this->visited_[0]) * this->get_bitmap_size_visited()));

    this->dev_cq_count_ = new PrefixSumGPU<int>(this->get_summary_size_v0());

    this->tmp_packet_max_length_ =
        sizeof(BitmapType) * this->get_bitmap_size_visited() / PACKET_LENGTH +
        omp_get_max_threads() * 2;
    this->tmp_packet_index_ = (bfs_detail::PacketIndex*)malloc(
        sizeof(bfs_detail::PacketIndex) * this->tmp_packet_max_length_);

    this->gpu_cq_comm_.local_buffer_ =
        (bfs_detail::CompressedStream*)page_aligned_xcalloc(
            sizeof(BitmapType) * this->get_bitmap_size_visited());
    CUDA_CHECK(cudaHostRegister(
        this->gpu_cq_comm_.local_buffer_,
        sizeof(BitmapType) * this->get_bitmap_size_visited(), 0));
#if VERTEX_SORTING
    this->gpu_visited_comm_.local_buffer_ =
        (bfs_detail::CompressedStream*)page_aligned_xcalloc(
            sizeof(BitmapType) * this->get_bitmap_size_visited());
    CUDA_CHECK(cudaHostRegister(
        this->gpu_visited_comm_.local_buffer_,
...
#endif
    this->gpu_cq_comm_.recv_buffer_ =
        (typename super_::ExpandCommBuffer*)page_aligned_xcalloc(
            sizeof(BitmapType) * this->get_bitmap_size_v0());
    CUDA_CHECK(cudaHostRegister(this->gpu_cq_comm_.recv_buffer_,
                                sizeof(BitmapType) * this->get_bitmap_size_v0(),
                                0));

    this->gpu_visited_comm_.recv_buffer_ =
        (typename super_::ExpandCommBuffer*)page_aligned_xcalloc(
            sizeof(BitmapType) * this->get_bitmap_size_v1());
    CUDA_CHECK(cudaHostRegister(this->gpu_visited_comm_.recv_buffer_,
                                sizeof(BitmapType) * this->get_bitmap_size_v1(),
                                0));

    this->thread_local_buffer_ = (typename super_::ThreadLocalBuffer**)malloc(
        sizeof(this->thread_local_buffer_[0]) * max_threads);
    this->d_ = (typename super_::DynamicDataSet*)malloc(sizeof(this->d_[0]));

    const int buffer_width =
        roundup<CACHE_LINE>(sizeof(typename super_::ThreadLocalBuffer) +
                            sizeof(bfs_detail::FoldPacket) * mpi.size_2dc);
    this->buffer_.thread_local_ =
        cache_aligned_xcalloc(buffer_width * max_threads);
    for (int i = 0; i < max_threads; ++i) {
      this->thread_local_buffer_[i] =
          (typename super_::ThreadLocalBuffer*)((uint8_t*)this->buffer_
                                                    .thread_local_ +
                                                buffer_width * i);
    }
    ...
    this->dev_fold_proc_buffer_ =
        (cuda::FoldGpuBuffer*)&this->dev_fold_io_buffer_[NUM_IO_BUFFERS];
    ...
    CUDA_CHECK(cudaMallocHost(
        (void**)&this->host_read_graph_buffer_,
        sizeof(this->host_read_graph_buffer_[0]) * num_stream_jobs_));

    // int num_extract_jobs =
    // get_blocks<EXTRACT_BLOCKS_PER_LAUNCH>(get_bitmap_size_v0());
    int64_t num_columns = this->get_number_of_edge_columns();
    int64_t index_size = this->graph_.row_starts_[num_columns];
    double elements_per_column = (double)index_size / num_columns;
    this->blocks_per_launch_ = (int)std::max(
        (GPU_PARAMS::BLOCKS_PER_LAUNCH_RATE / elements_per_column), 1.0);
  ...
  void deallocate_memory_gpu() {
    ...
    free(this->nq_bitmap_);
    this->nq_bitmap_ = NULL;

    CUDA_CHECK(cudaHostUnregister(this->nq_sorted_bitmap_));
    free(this->nq_sorted_bitmap_);
    this->nq_sorted_bitmap_ = NULL;
    ...
    // Device Memory
    CUDA_CHECK(cudaFree(this->pred_));
    this->pred_ = NULL;
    if (graph_on_gpu_) {
      CUDA_CHECK(cudaFree(this->dev_cq_bitmap_));
      this->dev_cq_bitmap_ = NULL;
      CUDA_CHECK(cudaFree(this->dev_cq_summary_));
      this->dev_cq_summary_ = NULL;
    } else {
      free(this->cq_bitmap_);
      this->cq_bitmap_ = NULL;
      free(this->cq_summary_);
      this->cq_summary_ = NULL;
    }
    CUDA_CHECK(cudaFree(this->dev_shared_visited_));
    this->dev_shared_visited_ = NULL;
    CUDA_CHECK(cudaFree(this->dev_nq_bitmap_));
    this->dev_nq_bitmap_ = NULL;
    CUDA_CHECK(cudaFree(this->dev_nq_sorted_bitmap_));
    this->dev_nq_sorted_bitmap_ = NULL;
    CUDA_CHECK(cudaFree(this->dev_visited_));
    this->dev_visited_ = NULL;

    delete this->dev_cq_count_;
    this->dev_cq_count_ = NULL;

    free(this->tmp_packet_index_);
    this->tmp_packet_index_ = NULL;

    CUDA_CHECK(cudaHostUnregister(this->gpu_cq_comm_.local_buffer_));
    free(this->gpu_cq_comm_.local_buffer_);
    this->gpu_cq_comm_.local_buffer_ = NULL;
#if VERTEX_SORTING
    CUDA_CHECK(cudaHostUnregister(this->gpu_visited_comm_.local_buffer_));
    free(this->gpu_visited_comm_.local_buffer_);
    this->gpu_visited_comm_.local_buffer_ = NULL;
#endif
    CUDA_CHECK(cudaHostUnregister(this->gpu_cq_comm_.recv_buffer_));
    free(this->gpu_cq_comm_.recv_buffer_);
    this->gpu_cq_comm_.recv_buffer_ = NULL;

    CUDA_CHECK(cudaHostUnregister(this->gpu_visited_comm_.recv_buffer_));
    free(this->gpu_visited_comm_.recv_buffer_);
    this->gpu_visited_comm_.recv_buffer_ = NULL;

    free(this->thread_local_buffer_);
    this->thread_local_buffer_ = NULL;
    free(this->d_);
    this->d_ = NULL;
    free(this->buffer_.thread_local_);
    this->buffer_.thread_local_ = NULL;

    CUDA_CHECK(cudaFreeHost(this->host_));
    this->host_ = NULL;
    CUDA_CHECK(cudaFree(this->dev_));
    this->dev_ = NULL;

    CUDA_CHECK(cudaFree(this->dev_buffer_));
    this->dev_buffer_ = NULL;
    this->dev_fold_io_buffer_ = NULL;
    this->dev_fold_proc_buffer_ = NULL;
    this->dev_column_buffer_ = NULL;
    this->dev_expand_buffer_ = NULL;

    CUDA_CHECK(cudaFreeHost(this->host_read_graph_buffer_));
    this->host_read_graph_buffer_ = NULL;

    for (int i = 0; i < NUM_CUDA_STREAMS; ++i) {
      CUDA_CHECK(cudaStreamDestroy(this->cuda_stream_array_[i]));
    }

    delete[] this->sched_.fold_end_job;
    this->sched_.fold_end_job = NULL;
  ...
  void initialize_memory_gpu() {
    ...
    cuda::cu_initialize_memory(
        ...
        this->get_bitmap_size_visited(), this->get_bitmap_size_v0(),
        this->get_bitmap_size_v1(), this->get_summary_size_v0(), this->pred_,
        this->dev_nq_bitmap_, this->dev_nq_sorted_bitmap_, this->dev_visited_,
        this->dev_cq_summary_, this->dev_cq_bitmap_, this->dev_shared_visited_,
        this->cuda_stream_array_, NUM_CUDA_STREAMS);
    CudaStreamManager::end_cuda();
...
#pragma omp for nowait
      ...
      bfs_detail::FoldPacket* packet_array =
          this->thread_local_buffer_[omp_get_thread_num()]->fold_packet;
      for (int i = 0; i < mpi.size_2dc; ++i) {
  ...
  void prepare_fold() {
    ...
    host_->nq_count = 0;
    CUDA_CHECK(cudaMemcpyAsync(&dev_->nq_count, &host_->nq_count,
                               sizeof(host_->nq_count), cudaMemcpyHostToDevice,
                               master_stream));
    ...
    if (graph_on_gpu_) {
      cuda::create_cq_list(dev_cq_bitmap_, dev_cq_summary_,
                           ...
                           dev_column_buffer_,
                           this->get_number_of_edge_columns(), master_stream);
      int* dev_last_offset =
          &dev_cq_count_->get_buffer()[this->get_summary_size_v0()];
      CUDA_CHECK(cudaMemcpyAsync(&host_->num_non_zero_columns, dev_last_offset,
                                 sizeof(dev_last_offset[0]),
                                 cudaMemcpyDeviceToHost, master_stream));
#if VERVOSE_MODE
    ...
    // begin receiver processing before starting fold main processing. It is OK,
    // I think.
  ...
  virtual int varint_encode(const int64_t* input, int length, uint8_t* output,
                            bfs_detail::VARINT_BFS_KIND kind) {
  ...
  struct GpuExpandCommCommand : public super_::ExpandCommCommand {
    GpuExpandCommCommand(ThisType* this__, bool cq_or_visited)
        : super_::ExpandCommCommand(this__, cq_or_visited), this_(this__) {
      //
    }
    virtual void comm() {
      this->gather_comm();
    ...
    void launch_update_kernel(const int4* index, LocalVertsIndex num_packet,
                              const uint8_t* v_stream,
                              LocalVertsIndex stream_length, uint32_t* target,
                              uint32_t* summary) {
      // launch kernel
      int stream_number =
          (this_->expand_job_id_++) % std::min<int>(2, NUM_CUDA_STREAMS);
      int4* dev_packet_list =
          this_->dev_expand_buffer_[stream_number].packet_list;
      ...
      int log_src_factor =
          this_->log_local_bitmap_ + super_::LOG_PACKING_EDGE_LISTS;
#if 0
      ...
      // 		 To process with unsigned coding, modify the processing
      // of encoding.
      cuda::decode_varint_stream_signed(dev_packet_list, dev_v_stream,
                                        num_packet, (int32_t*)dev_v_list,
                                        cuda_stream);
#if DEBUG_PRINT
      ...
      if (num_edges > 0) {
        CUDA_CHECK(
            cudaMallocHost((void**)&host_v0_list, num_edges * sizeof(int64_t)));
        CUDA_CHECK(cudaThreadSynchronize());
        CUDA_CHECK(cudaMemcpy(host_v0_list, dev_v_list,
                              num_edges * sizeof(int64_t),
                              cudaMemcpyDeviceToHost));
...
#endif
      cuda::update_bitmap(dev_packet_list, num_packet, dev_v_list, target,
                          summary, log_src_factor, cuda_stream);
#if VERVOSE_MODE
      ++this_->kernel_launch_update_bitmap_;
#endif
    }
    void gpu_update_from_stream(const uint8_t* byte_stream, int* offset,
                                int* count, int num_src, uint32_t* target,
                                uint32_t* summary) {
      ...
      for (int i = 0; i < num_src; ++i) {
        int index_start =
            ((bfs_detail::CompressedStream*)(byte_stream + offset[i]))
                ->packet_index_start;
        packet_count[i] =
            (count[i] - index_start * sizeof(bfs_detail::PacketIndex) -
             offsetof(bfs_detail::CompressedStream, d)) /
            sizeof(bfs_detail::PacketIndex);
...
#if DEBUG_PRINT
      if (mpi.isMaster())
        printf("L:%d:num_total_packets=%" PRId64 "\n", __LINE__,
               num_total_packets);
#endif
      ...
      int4* index;
      CUDA_CHECK(
          cudaMallocHost((void**)&index, sizeof(index[0]) * num_total_packets));
      ...
      for (int i = 0; i < num_src; ++i) {
        ...
        bfs_detail::PacketIndex* packet_index =
            &stream->d.index[stream->packet_index_start];
        int64_t stream_offset = offset[i] +
                                offsetof(bfs_detail::CompressedStream, d) -
                                base_stream_offset;
        ...
        for (int64_t k = 0; k < packet_count[i]; ++k) {
          ...
          if (num_packet == GPU_PARAMS::EXPAND_PACKET_LIST_LENGTH ||
            ...
            launch_update_kernel(index + base_i, num_packet,
                                 byte_stream + base_stream_offset,
                                 stream_offset, target, summary);
          ...
          index[base + k] =
              make_int4(stream_offset, packet_index[k].num_int, v_offset, i);

          v_offset += num_int;
          stream_offset += length;
        }
        assert((roundup<sizeof(bfs_detail::PacketIndex)>(
                    (stream_offset + base_stream_offset - offset[i] -
                     offsetof(bfs_detail::CompressedStream, d))) /
      ...
      if (num_packet > 0) {
        launch_update_kernel(index + base_i, num_packet,
                             byte_stream + base_stream_offset, stream_offset,
                             target, summary);
      }
      ...
      if (this->cq_or_visited_) {
        ...
        if (graph_on_gpu_) {
          if (this->stream_or_bitmap_) {
            // stream update
            gpu_update_from_stream((uint8_t*)this->recv_buffer_, this->offset_,
                                   this->count_, this->comm_size_,
                                   this_->dev_cq_bitmap_,
                                   this_->dev_cq_summary_);
          } else {
            int64_t bitmap_size = this_->get_bitmap_size_v0();
            CUDA_CHECK(cudaMemcpy(
                this_->dev_cq_bitmap_, this->recv_buffer_->bitmap,
                sizeof(BitmapType) * bitmap_size, cudaMemcpyHostToDevice));
            // fill 1 to summary
            cuda::fill_1_summary(this_->dev_cq_summary_,
                                 this_->get_summary_size_v0());
#if VERVOSE_MODE
          ...
          if (this->stream_or_bitmap_) {
            ...
            // bitmap
            this_->d_->num_vertices_in_cq_ =
                this_->get_bitmap_size_v0() * super_::NUMBER_PACKING_EDGE_LISTS;
            // fill 1 to summary
        ...
        if (this->stream_or_bitmap_) {
          ...
          gpu_update_from_stream((uint8_t*)this->recv_buffer_, this->offset_,
                                 this->count_, this->comm_size_,
                                 this_->dev_shared_visited_, NULL);
        } else {
          int64_t bitmap_size = this_->get_bitmap_size_v1();
          CUDA_CHECK(cudaMemcpy(
              this_->dev_shared_visited_, this->recv_buffer_->bitmap,
              sizeof(BitmapType) * bitmap_size, cudaMemcpyHostToDevice));
#if VERVOSE_MODE
          this_->cuda_io_bytes_ += sizeof(BitmapType) * bitmap_size;
#endif
...
#if VERVOSE_MODE
  double kernel_time_create_cq_list_;
  int kernel_launch_create_cq_list_;
  double kernel_time_read_graph_;
  int kernel_launch_read_graph_;
  int retry_count_read_graph_;
  double kernel_time_filter_edge_;
  double cuda_time_extract_;
  double kernel_time_receive_process_;
  int kernel_launch_receive_process_;
  double kernel_time_update_bitmap_;
