...
// Since communication and computation is overlapped, we cannot have main thread
// do both tasks.
...
// 0: completely off, 1: only reduce isolated vertices, 2: sort by degree and
// reduce isolated vertices
...
#define COMPRESS_ROW_STARTS (1 & ISOLATE_FIRST_EDGE)
...
// WARNING: In the most case, BFS generates correct answer without initializing
// predecessor array because all the vertexes reached in the previous would be
// reached in the current run. But this is not true in the general case. BFS may
// generate wrong answer in some situation.
...
enum {
  ...
  EXPAND_PACKET_LIST_LENGTH =
      EXPAND_DECODE_BLOCK_LENGTH / PRM::PACKET_LENGTH * 2,
...
struct auto_tuning_t {
  ...
  bool operator<(const auto_tuning_t& x) { return val < x.val; }
};
