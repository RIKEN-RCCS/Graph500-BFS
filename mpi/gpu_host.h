...
// If you are using "compute-exclusive-thread mode" as a CUDA driver compute
// mode, you need to turn on this flag. This requires linking to Driver API
// Library.
...
#ifndef CUDA_CHECK
static void cuda_error_handler(cudaError_t error, const char* srcfile,
                               int linenumber) {
  char hostname[256];
  ...
  printf("cudaError:%s\n%s(PID:%d,Rank:%d) %s(%d)\n", cudaGetErrorString(error),
         hostname, pid, rank, srcfile, linenumber);
#else
  printf("cudaError:%s\n%s(PID:%d) %s(%d)\n", cudaGetErrorString(error),
         hostname, pid, srcfile, linenumber);
#endif
  throw "cudaError";
}
#define CUDA_CHECK(method)                         \
  if (cudaError_t error = (cudaError_t)method) {   \
...
class CudaCommand {
 public:
  ...
  virtual bool init() {
    return true;
  }  // To invalidate this command, return false.
...
class CudaStreamManager {
  ...
  CudaStreamManager(int device_index)
    ...
    if (device_index_ != -1) {
      ...
      for (int i = 0; i < dev_count; ++i) {
        if (cuCtxCreate(&ctx_, CU_CTX_SCHED_AUTO, i)) continue;
        device_index_ = i;
        break;
      }
      if (device_index_ == -1) {
 ...
 public:
  ...
  static CudaStreamManager* get_instance() { return instance; }
