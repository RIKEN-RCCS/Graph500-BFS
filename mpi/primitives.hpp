...
template <>
struct MpiTypeOf<UnweightedEdge> {
...
template <>
struct MpiTypeOf<UnweightedPackedEdge> {
  static MPI_Datatype type;
};
...
struct UnweightedPackedEdge {
  ...
  int64_t v0() const {
    return (v0_low_ | (static_cast<int64_t>(high_ & 0xFFFF) << 32));
  }
  int64_t v1() const {
    return (v1_low_ | (static_cast<int64_t>(high_ >> 16) << 32));
  }
  void set(int64_t v0, int64_t v1) {
  ...
  static void initialize() {
    MPI_Type_contiguous(3, MPI_UINT32_T,
                        &MpiTypeOf<UnweightedPackedEdge>::type);
...
struct WeightedEdge {
  ...
  static void initialize() {
    ...
    MPI_Aint displs[] = {
        reinterpret_cast<MPI_Aint>(
            &(static_cast<WeightedEdge*>(NULL)->v0_low_)),
        reinterpret_cast<MPI_Aint>(
            &(static_cast<WeightedEdge*>(NULL)->v1_low_)),
        reinterpret_cast<MPI_Aint>(&(static_cast<WeightedEdge*>(NULL)->high_)),
        reinterpret_cast<MPI_Aint>(
            &(static_cast<WeightedEdge*>(NULL)->weight_))};
    MPI_Datatype types[] = {MPI_UINT32_T, MPI_UINT32_T, MPI_UINT32_T, MPI_INT};
    MPI_Type_create_struct(4, block_length, displs, types,
                           &MpiTypeOf<WeightedEdge>::type);
    MPI_Type_commit(&MpiTypeOf<WeightedEdge>::type);
  }

  static void uninitialize() { MPI_Type_free(&MpiTypeOf<WeightedEdge>::type); }
};
...
class Rail40bit {
 public:
  ...
  void alloc(int64_t length) {
    ...
    rail_ = static_cast<Rail40bitElement*>(
        cache_aligned_xmalloc(alloc_length * sizeof(Rail40bitElement)));
#ifndef NDEBUG
...
class Pack48bit {
 public:
  ...
  void alloc(int64_t length) {
    i32_ =
        static_cast<int32_t*>(cache_aligned_xmalloc(length * sizeof(int32_t)));
    i16_ = static_cast<uint16_t*>(
        cache_aligned_xmalloc(length * sizeof(uint16_t)));
#ifndef NDEBUG
...
class Rail48bit {
 public:
  ...
  ~Rail48bit() { this->free(); }
  void alloc(int64_t length) {
    rail_ = static_cast<uint16_t*>(
        cache_aligned_xmalloc(length * sizeof(uint16_t) * 3));
#ifndef NDEBUG
  ...
  void copy_from(int64_t to, Rail48bit& array, int64_t from, int64_t size) {
    memmove(rail_ + to * 3, array.rail_ + from * 3,
            sizeof(rail_[0]) * size * 3);
  }
...
class Rail32bit {
 public:
  ...
  ~Rail32bit() { this->free(); }
  void alloc(int64_t length) {
    rail_ = static_cast<uint32_t*>(
        cache_aligned_xmalloc(length * sizeof(uint32_t)));
#ifndef NDEBUG
...
class Pack64bit {
 public:
  ...
  void alloc(int64_t length) {
    i64_ =
        static_cast<int64_t*>(cache_aligned_xmalloc(length * sizeof(int64_t)));
#ifndef NDEBUG
