...
gather* init_gather(void* input, size_t input_count, size_t elt_size,
  ...
  g->local_indices =
      (size_t*)page_aligned_xmalloc(nrequests_max * sizeof(size_t));
  g->remote_ranks = (int*)page_aligned_xmalloc(nrequests_max * sizeof(int));
  g->remote_indices =
      (MPI_Aint*)page_aligned_xmalloc(nrequests_max * sizeof(MPI_Aint));
...
void destroy_gather(gather* g) {
  assert(!g->valid);
  free(g->local_indices);
  g->local_indices = NULL;
  free(g->remote_ranks);
  g->remote_ranks = NULL;
  free(g->remote_indices);
  g->remote_indices = NULL;
...
void begin_gather(gather* g) {
  assert(!g->valid);
  {
    ...
    for (i = 0; i < nr; ++i) g->remote_ranks[i] = g->comm_size;
  }
  g->valid = 1;
}
 ...
 * */
void histogram_sort_size_tMPI_Aint(int* restrict keys,
                                   const int* restrict rowstart, int numkeys,
                                   size_t* restrict values1,
                                   MPI_Aint* restrict values2) {
  int* restrict insert_positions =
      (int*)cache_aligned_xmalloc(numkeys * sizeof(int));
  ...
  for (i = 0; i < rowstart[numkeys]; ++i) {
    ...
    while (!(i >= rowstart[key] && i < insert_positions[key])) {
      ...
      // print_with_prefix("target_pos = %d from limit %d", target_pos,
      // rowstart[key + 1]);
      if (target_pos == i) continue;
      assert(target_pos < rowstart[key + 1]);
      // print_with_prefix("swapping [%d] = (%d, %d, %d) with [%d] = (%d, %d,
      // %d)", i, keys[i], (int)values1[i], (int)values2[i], target_pos,
      // keys[target_pos], (int)values1[target_pos], (int)values2[target_pos]);
      {
        int t = keys[i];
        key = keys[target_pos];
        keys[i] = key;
      ...
      {
        size_t t = values1[i];
        values1[i] = values1[target_pos];
        values1[target_pos] = t;
      }
      {
        MPI_Aint t = values2[i];
        values2[i] = values2[target_pos];
        values2[target_pos] = t;
      }
      assert(key >= 0 && key < numkeys);
    }
...
void end_gather(gather* g) {
  ...
  histogram_sort_size_tMPI_Aint(remote_ranks, send_offsets, size + 1,
                                local_indices, remote_indices);
  assert(send_offsets[size] == send_offsets[size + 1] ||
         remote_ranks[send_offsets[size]] == size);
  ...
  MPI_Aint* restrict recv_data =
      (MPI_Aint*)page_aligned_xmalloc(recv_offsets[size] * sizeof(MPI_Aint));
  MPI_Alltoallv(remote_indices, send_counts, send_offsets, MPI_AINT, recv_data,
                recv_counts, recv_offsets, MPI_AINT, comm);
  char* restrict reply_data =
      (char*)page_aligned_xmalloc(recv_offsets[size] * elt_size);
  for (i = 0; i < (size_t)recv_offsets[size]; ++i) {
    assert(recv_data[i] >= 0 && recv_data[i] < (MPI_Aint)input_count);
    memcpy(reply_data + i * elt_size, input + recv_data[i] * elt_size,
           elt_size);
  }
  free(recv_data);
  char* restrict recv_reply_data =
      (char*)page_aligned_xmalloc(send_offsets[size] * elt_size);
  MPI_Alltoallv(reply_data, recv_counts, recv_offsets, datatype,
                recv_reply_data, send_counts, send_offsets, datatype, comm);
  free(reply_data);
  for (i = 0; i < nrequests_max; ++i) {
    if (remote_ranks[i] >= 0 && remote_ranks[i] < size) {
      memcpy(output + local_indices[i] * elt_size,
             recv_reply_data + i * elt_size, elt_size);
    }
  }
...
scatter_constant* init_scatter_constant(void* array, size_t array_count,
                                        ...
                                        size_t nrequests_max,
                                        MPI_Datatype dt /* unused */) {
  (void)dt;
  scatter_constant* sc =
      (scatter_constant*)cache_aligned_xmalloc(sizeof(scatter_constant));
  sc->array = array;
...
void destroy_scatter_constant(scatter_constant* sc) {
  assert(!sc->valid);
  free(sc->remote_ranks);
  sc->remote_ranks = NULL;
  free(sc->remote_indices);
  sc->remote_indices = NULL;
  MPI_Comm_free(&sc->comm);
  free(sc->send_counts);
  sc->send_counts = NULL;
  free(sc->send_offsets);
  sc->send_offsets = NULL;
  free(sc->recv_counts);
  sc->recv_counts = NULL;
  free(sc->recv_offsets);
  sc->recv_offsets = NULL;
  free(sc);
}
...
void begin_scatter_constant(scatter_constant* sc) {
  assert(!sc->valid);
  {
    size_t i, nr = sc->nrequests_max;
    for (i = 0; i < nr; ++i) sc->remote_ranks[i] = sc->comm_size;
  }
  sc->valid = 1;
}
...
/* Adapted from histogram_sort_inplace in boost/graph/detail/histogram_sort.hpp
 * */
void histogram_sort_MPI_Aint(int* restrict keys, const int* restrict rowstart,
                             int numkeys, MPI_Aint* restrict values1) {
  int* restrict insert_positions =
      (int*)cache_aligned_xmalloc(numkeys * sizeof(int));
  memcpy(insert_positions, rowstart, numkeys * sizeof(int));
  int i;
...
/* Adapted from histogram_sort_inplace in boost/graph/detail/histogram_sort.hpp
 * */
void histogram_sort_MPI_Aintcharblock(int* restrict keys,
                                      const int* restrict rowstart, int numkeys,
  ...
  for (i = 0; i < rowstart[numkeys]; ++i) {
    ...
    while (!(i >= rowstart[key] && i < insert_positions[key])) {
      ...
      {
        char t[elt_size2];
        memcpy(t, values2 + i * elt_size2, elt_size2);
        memcpy(values2 + i * elt_size2, values2 + target_pos * elt_size2,
               elt_size2);
        memcpy(values2 + target_pos * elt_size2, t, elt_size2);
      }
      assert(key >= 0 && key < numkeys);
    }
    print_with_prefix("done");
...
void end_scatter(scatter* sc) {
  ...
  histogram_sort_MPI_Aintcharblock(remote_ranks, send_offsets, size + 1,
                                   remote_indices, send_data, elt_size);
  MPI_Alltoall(send_counts, 1, MPI_INT, recv_counts, 1, MPI_INT, comm);
  recv_offsets[0] = 0;
  for (i = 0; i < (size_t)size; ++i) {
    recv_offsets[i + 1] = recv_offsets[i] + recv_counts[i];
  }
  MPI_Aint* restrict recv_indices =
      (MPI_Aint*)page_aligned_xmalloc(recv_offsets[size] * sizeof(MPI_Aint));
  char* restrict recv_data =
      (char*)cache_aligned_xmalloc(recv_offsets[size] * elt_size);
  MPI_Alltoallv(remote_indices, send_counts, send_offsets, MPI_AINT,
                recv_indices, recv_counts, recv_offsets, MPI_AINT, comm);
  MPI_Alltoallv(send_data, send_counts, send_offsets, sc->datatype, recv_data,
                recv_counts, recv_offsets, sc->datatype, comm);
  for (i = 0; i < (size_t)recv_offsets[size]; ++i) {
    assert(recv_indices[i] >= 0 && recv_indices[i] < (MPI_Aint)array_count);
    memcpy(array + recv_indices[i] * elt_size, recv_data + i * elt_size,
           elt_size);
  }
  free(recv_data);
  free(recv_indices);
...
class BfsValidation {
 ...
 public:
  ...
  /* Returns true if result is valid.  Also, updates high 16 bits of each
   * element of pred to contain the BFS level number (or -1 if not visited) of
   * each vertex; this is based on the predecessor map if the user didn't
   * provide it.
   * */
  template <typename EdgeList>
  bool validate(EdgeList* edge_list, const int64_t root, int64_t* const pred,
    ...
    *edge_visit_count_ptr = 0; /* Ensure it is a valid pointer */
    ...
    if (root < 0 || root >= nglobalverts) {
      print_with_prefix("Validation error: root vertex %" PRId64 " is invalid.",
                        root);
    }
    ...
    if (root_is_mine) {
      ...
      if (get_pred_from_pred_entry(pred[root_local]) != root) {
        print_with_prefix("Validation error: parent of root vertex %" PRId64
                          " is %" PRId64 ", not the root itself.",
                          root, get_pred_from_pred_entry(pred[root_local]));
        ++error_counts;
      }
    }
    if (error_counts) return false; /* Fail */
    ...
    {
      int* restrict pred_owner = (int*)cache_aligned_xmalloc(
          std::min(chunksize_, nlocalverts) * sizeof(int));
      int64_t* restrict pred_local = (int64_t*)cache_aligned_xmalloc(
          std::min(chunksize_, nlocalverts) * sizeof(int64_t));
...
#pragma omp parallel for
        for (ptrdiff_t i = i_start; i < i_end; ++i) {
          if ((!root_is_mine || i != root_local) &&
            ...
            if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
              print_with_prefix(
                  "Validation error: parent of non-root vertex %" PRId64
                  " is itself.",
                  i * mpi.size_2d + mpi.rank_2d);
          }
        }
      }
      free(pred_owner);
...

    {
      ...
      unsigned char* restrict pred_valid =
          (unsigned char*)cache_aligned_xmalloc(nlocalverts *
      ...
      for (int loop_count = 0; loop_count < num_loops && error_counts == 0;
           ++loop_count) {
        EdgeType* edge_data;
        const int bufsize = edge_list->read(&edge_data);
        assert(bufsize <= chunksize_);
        // begin_gather(pred_win);
        int* restrict local_indices_r =
            (int*)cache_aligned_xmalloc(chunksize_ * sizeof(int));
        int* restrict remote_indices_r =
            (int*)page_aligned_xmalloc(chunksize_ * sizeof(int));
        int* restrict local_indices_c =
            (int*)cache_aligned_xmalloc(chunksize_ * sizeof(int));
        int* restrict remote_indices_c =
            (int*)page_aligned_xmalloc(chunksize_ * sizeof(int));
#pragma omp parallel
        {
...
#pragma omp for schedule(static)  // ***
          for (int i = 0; i < bufsize; ++i) {
            ...
            // add_gather_request(pred_win, i * 2 + 0, edge_owner[i * 2 + 0],
            // edge_local[i * 2 + 0], i * 2 + 0); add_gather_request(pred_win, i
            // * 2 + 1, edge_owner[i * 2 + 1], edge_local[i * 2 + 1], i * 2 +
            // 1);
          }
        }  // #pragma omp parallel
        ...
        free(remote_indices_r);
        remote_indices_r = NULL;
        free(remote_indices_c);
        remote_indices_c = NULL;
        int64_t* restrict reply_data_r =
            (int64_t*)page_aligned_xmalloc(recv_count_r * sizeof(int64_t));
        int64_t* restrict reply_data_c =
            (int64_t*)page_aligned_xmalloc(recv_count_c * sizeof(int64_t));
...
#pragma omp parallel for
        ...
        scatter_r.free(reply_indices_r);
        reply_indices_r = NULL;
        scatter_c.free(reply_indices_c);
        reply_indices_c = NULL;
        assert(scatter_r.get_send_count() == bufsize);
        assert(scatter_c.get_send_count() == bufsize);
        ...
        int64_t* restrict edge_preds =
            (int64_t*)cache_aligned_xmalloc(2 * chunksize_ * sizeof(int64_t));
...
#pragma omp parallel for
        ...
        // end_gather(pred_win);
        free(local_indices_r);
        local_indices_r = NULL;
        free(local_indices_c);
        local_indices_c = NULL;
        scatter_r.free(recv_pred_r);
        recv_pred_r = NULL;
        scatter_r.free(recv_pred_c);
        recv_pred_c = NULL;

        // begin_scatter_constant(pred_valid_win);
        MPI_Aint* restrict remote_valid_indices_r =
            (MPI_Aint*)page_aligned_xmalloc(chunksize_ * sizeof(MPI_Aint));
        MPI_Aint* restrict remote_valid_indices_c =
            (MPI_Aint*)page_aligned_xmalloc(chunksize_ * sizeof(MPI_Aint));
...
#pragma omp for schedule(static) reduction(+ : edge_visit_count)  // ***
          for (int i = 0; i < bufsize; ++i) {
            ...
            uint16_t src_depth =
                get_depth_from_pred_entry(edge_preds[i * 2 + 0]);
            uint16_t tgt_depth =
                get_depth_from_pred_entry(edge_preds[i * 2 + 1]);
            if (src_depth != UINT16_MAX && tgt_depth == UINT16_MAX) {
              if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
                print_with_prefix(
                    "Validation error: edge connects vertex %" PRId64
                    " in the BFS tree (depth %" PRIu16 ") to vertex %" PRId64
                    " outside the tree.",
                    src, src_depth, tgt);
              ...
              if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
                print_with_prefix(
                    "Validation error: depths of edge endpoints %" PRId64
                    " (depth %" PRIu16 ") and %" PRId64 " (depth %" PRIu16
                    ") are too far apart (abs. val. > 1).",
                    src, src_depth, tgt, tgt_depth);
...
#pragma omp for schedule(static)  // ***
          for (int i = 0; i < bufsize; ++i) {
            ...
            int64_t tgt = edge_data[i].v1();
            if (get_pred_from_pred_entry(edge_preds[i * 2 + 0]) == tgt) {
              remote_valid_indices_r[offsets_r[vertex_owner_c(src)]++] =
                  vertex_local(src);
            }
            if (get_pred_from_pred_entry(edge_preds[i * 2 + 1]) == src) {
              remote_valid_indices_c[offsets_c[vertex_owner_r(tgt)]++] =
                  vertex_local(tgt);
            }
          }  // #pragma omp for schedule(static)
        }  // #pragma omp parallel
        ...
        MPI_Aint* restrict recv_valid_indices_c =
            scatter_c.scatter(remote_valid_indices_c);
        recv_count_c = scatter_c.get_recv_count();
        free(remote_valid_indices_r);
        remote_valid_indices_r = NULL;
        free(remote_valid_indices_c);
        remote_valid_indices_c = NULL;
...
#pragma omp parallel for
      for (i = 0; i < (ptrdiff_t)nlocalverts; ++i) {
        ...
        if (root_owner == mpi.rank_2d && root_local == i)
          found_pred_edge = 1; /* Root vertex */
        if (!found_pred_edge) {
          int64_t v = i * mpi.size_2d + mpi.rank_2d;
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix(
                "Validation error: no graph edge from vertex %" PRId64
                " to its parent %" PRId64 ".",
                v, get_pred_from_pred_entry(pred[i]));
        }
      }
      free(pred_valid);

      MPI_Allreduce(MPI_IN_PLACE, &edge_visit_count, 1, MPI_INT64_T, MPI_SUM,
                    mpi.comm_2d);
 ...
 private:
  ...
  int64_t check_value_ranges(const int64_t nglobalverts,
...
#pragma omp parallel for
      for (ptrdiff_t i = i_start; i < i_end; ++i) {
        ...
        if (p < -1 || p >= nglobalverts) {
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix("Validation error: parent of vertex %" PRId64
                              " is out-of-range value %" PRId64 ".",
                              i * mpi.size_2d + mpi.rank_2d, p);
        }
      }
    }
    return error_counts;
...
#pragma omp parallel for
    ...
    int64_t* restrict pred_pred = (int64_t*)xMPI_Alloc_mem(
        ...
        sizeof(int64_t)); /* Predecessor info of predecessor vertex for each
                             local vertex */
    gather* pred_win =
        init_gather((void*)pred, nlocalverts, sizeof(int64_t), pred_pred,
                    std::min(chunksize_, nlocalverts),
                    std::min(chunksize_, nlocalverts), MPI_INT64_T);
    int* restrict pred_owner = (int*)cache_aligned_xmalloc(
        std::min(chunksize_, nlocalverts) * sizeof(int));
    int64_t* restrict pred_local = (int64_t*)cache_aligned_xmalloc(
        std::min(chunksize_, nlocalverts) * sizeof(int64_t));
    int iter_number = 0;
    {
      /* Iteratively update depth[v] = min(depth[v], depth[pred[v]] + 1)
       * [saturating at UINT16_MAX] until no changes. */
      while (1) {
        ++iter_number;
        int any_changes = 0;
...
#pragma omp parallel for reduction(|| : any_changes)
          for (i = i_start; i < i_end; ++i) {
            ...
            if (get_depth_from_pred_entry(pred_pred[i - i_start]) !=
              ...
              if (get_depth_from_pred_entry(pred[i]) != UINT16_MAX &&
                ...
                if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
                  print_with_prefix(
                      "Validation error: BFS predecessors do not form a tree; "
                      "see vertices %" PRId64 " (depth %" PRIu16
                      ") and %" PRId64 " (depth %" PRIu16 ").",
                      i * mpi.size_2d + mpi.rank_2d,
                      get_depth_from_pred_entry(pred[i]),
        ...
        MPI_Allreduce(MPI_IN_PLACE, &any_changes, 1, MPI_INT, MPI_LOR,
                      mpi.comm_2d);
  ...
  int64_t check_bfs_depth_map_using_predecessors(const int64_t root,
                                                 const int64_t* const pred) {
...

    {
      if (root_is_mine && get_depth_from_pred_entry(pred[root_local]) != 0) {
        print_with_prefix("Validation error: depth of root vertex %" PRId64
                          " is %" PRIu16 ", not 0.",
                          root, get_depth_from_pred_entry(pred[root_local]));
        ++error_counts;
      }
#pragma omp parallel for
      for (int64_t i = 0; i < nlocalverts; ++i) {
        if (get_pred_from_pred_entry(pred[i]) == -1 &&
          ...
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix("Validation error: depth of vertex %" PRId64
                              " with no predecessor is %" PRIu16
                              ", not UINT16_MAX.",
                              i * mpi.size_2d + mpi.rank_2d,
                              get_depth_from_pred_entry(pred[i]));
          ...
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix(
                "Validation error: predecessor of claimed unreachable vertex "
    ...
    for (int64_t ii = 0; ii < maxlocalverts; ii += chunksize_) {
      ...
      assert(i_end - i_start >= 0 &&
             i_end - i_start <= (ptrdiff_t)std::min(chunksize_, nlocalverts));
#pragma omp parallel for
      for (ptrdiff_t i = i_start; i < i_end; ++i) {
        int64_t v = get_pred_from_pred_entry(pred[i]);
        pred_owner[i - i_start] = vertex_owner(v);
...
#pragma omp parallel for
      for (ptrdiff_t i = i_start; i < i_end; ++i) {
        ...
        if (get_depth_from_pred_entry(pred_pred[i - i_start]) == UINT16_MAX) {
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix("Validation error: predecessor %" PRId64
                              " of vertex %" PRId64 " (depth %" PRIu16
        ...
        if (get_depth_from_pred_entry(pred[i]) !=
          ...
          if (__sync_fetch_and_add(&error_counts, 1) < MAX_OUTPUT)
            print_with_prefix(
                ...
                "vertices %" PRId64 " (depth %" PRIu16 ") and %" PRId64
                " (depth %" PRIu16 ").",
                i * mpi.size_2d + mpi.rank_2d,
                get_depth_from_pred_entry(pred[i]),
                get_pred_from_pred_entry(pred[i]),
