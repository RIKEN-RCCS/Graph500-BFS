...
#define USER_START(s)                             \
  SCOREP_USER_REGION_DEFINE(scorep_region_##s);   \
  SCOREP_USER_REGION_BEGIN(scorep_region_##s, #s, \
...
extern "C" void user_defined_proc(const int* FLAG, const char* NAME,
                                  const int* LINE, const int* THREAD);
...
#define USER_START(s)                                \
  do {                                               \
    int line = __LINE__;                             \
    int flag = 102;                                  \
    user_defined_proc(&flag, __FILE__, &line, NULL); \
  } while (false)
#define USER_END(s)                                  \
  do {                                               \
    int line = __LINE__;                             \
...
#define DEBUG_PRINT_SWITCH_2(...)                       \
  do {                                                  \
    if (mpi.isMaster()) print_with_prefix(__VA_ARGS__); \
  } while (0)
...
#define debug_print(prefix, ...) \
  MAKE_DEBUG_PRINT_SWITCH(DEBUG_PRINT_##prefix)(#prefix " " __VA_ARGS__)
#else
#define debug_print(prefix, ...)
#endif
...
struct GLOBAL_CLOCK {
  ...
  int64_t get() { return get_time_in_microsecond() - clock_start; }
};
...
void throw_exception(const char* format, ...) {
  ...
  va_end(arg);
#if PRINT_WITH_TIME
  fprintf(IMD_OUT, "[r:%d,%f] %s\n", mpi.rank, global_clock.get() / 1000000.0,
          buf);
#else
...
void x_allocate_check(void* ptr) {
  ...
  if (mpi.isMaster() && nbytes > 1024 * 1024) {
    fprintf(IMD_OUT, "[MEM] %f MB (+ %f MB)\n",
            (double)g_memory_usage / (1024 * 1024),
...
void print_max_memory_usage() {
  int64_t g_max = 0;
  MPI_Reduce(&g_memory_usage, &g_max, 1, MpiTypeOf<int64_t>::type, MPI_MAX, 0,
             mpi.comm_2d);
  if (mpi.isMaster()) {
...
void* xMPI_Alloc_mem(size_t nbytes) {
  ...
  if (nbytes != 0 && !p) {
    throw_exception("MPI_Alloc_mem failed for size%zu (%" PRId64 ") byte(s)",
...
void* cache_aligned_xcalloc(const size_t size) {
  ...
  if (posix_memalign(&p, CACHE_LINE, size)) {
    throw_exception("Out of memory trying to allocate %zu (%" PRId64
                    ") byte(s)",
                    size, (int64_t)size);
  }
...
void* shared_malloc(size_t nbytes) {
  ...
  if (rank == 0) {
    ...
    for (int i = 0; i < 1000; ++i) {
      shmid = shmget(++shm_key, nbytes, IPC_CREAT | IPC_EXCL | 0600);
      if (shmid != -1) break;
#ifndef NDEBUG
      perror("shmget try");
#endif
...
void test_shared_memory() {
  ...
  shared_free(mem);
  MPI_Allreduce(&result, &global_result, 1, MpiTypeOf<int>::type, MPI_LOR,
                MPI_COMM_WORLD);
  if (global_result == false) {
    if (mpi.isMaster())
      print_with_prefix(
          "Shared memory test failed!! Please, check MPI_NUM_NODE.");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
}
#else   // #if SHARED_MEMORY
void* shared_malloc(size_t size) { return page_aligned_xcalloc(size); }
void shared_free(void* shm) { free(shm); }
...
template <typename T>
T roundup(T size, T width) {
  return (size + width - 1) / width * width;
}

template <typename T>
T get_blocks(T size, T width) {
  return (size + width - 1) / width;
}

template <typename T>
T roundup_2n(T size, T width) {
  return (size + width - 1) & -width;
}
...
template <typename T>
void get_partition(T* blk_offset, int num_blks, int parts_per_blk, int part_idx,
                   T& begin, T& end) {
  int blk_idx = part_idx / parts_per_blk;
  T blk_begin = blk_offset[blk_idx];
  T blk_size = blk_offset[blk_idx + 1] - blk_begin;
  get_partition(blk_size, parts_per_blk, part_idx - blk_idx * parts_per_blk,
                begin, end);
  begin += blk_begin;
  end += blk_begin;
}
...
template <typename T>
void get_partition(int64_t size, T* sorted, int log_blk, int num_part,
                   int part_idx, int64_t& begin, int64_t& end) {
  if (size == 0) {
    begin = end = 0;
    return;
  }
...
/*
template <typename T>
void get_partition(int64_t size, T* sorted, int64_t min_value, int64_t
max_value, int64_t min_blk_size, int num_part, int part_idx, T*& begin, T*& end)
{
        T blk_size = std::max(min_blk_size, (max_value - min_value + num_part -
1) / num_part); int64_t begin_value = min_value + blk_size * part_idx; int64_t
end_value = begin_value + blk_size; begin = std::lower_bound(sorted, sorted +
size, begin_value); end = std::lower_bound(sorted, sorted + size, end_value);
}
*/
...
void print_current_binding(const char* message) {
  ...
  print_with_prefix("th:%d %s -> [%s]", thread_id, message,
                    str_affinity.c_str());
}
...
void initialize_num_threads() {
  ...
  num_bfs_threads =
      num_omp_threads;  // currently there are no background thread
...
class CoreBinding {
 public:
  CoreBinding() { num_procs = sysconf(_SC_NPROCESSORS_CONF); }
  virtual ~CoreBinding() {}
...
class SimpleCoreBinding : public CoreBinding {
 public:
  SimpleCoreBinding() : CoreBinding() {}

  virtual int cpu(int tid) { return (tid % num_procs); }
  virtual int num_logical_CPUs() { return num_procs; }
};

class ManualCoreBinding : public CoreBinding {
 public:
  ManualCoreBinding(int* cpuset, int num_avail_procs) : CoreBinding() {
    avail_procs.insert(avail_procs.begin(), cpuset, cpuset + num_avail_procs);
  }

  virtual int cpu(int tid) { return avail_procs[tid]; }
  virtual int num_logical_CPUs() { return avail_procs.size(); }
 ...
 public:
  AutoDetectCoreBinding(int numa_node_size, int numa_node_rank)
    ...
    if (memcmp(vendor_sign.ch, "GenuineIntel", 12) == 0) {
      ...
      if (mpi.isMaster())
        print_with_prefix("Error: Unknown CPU: %s", vendor_sign.ch);
    }
    ...
    num_logical_CPUs_within_core =
        std::lower_bound(core_list, core_list + num_procs,
                         core_list[0] + (1 << (logical_CPU_bits + 16))) -
        core_list;
    int logical_CPUs_within_numa =
        std::lower_bound(
            core_list, core_list + num_procs,
            core_list[0] + (1 << (core_bits + logical_CPU_bits + 16))) -
        core_list;
    num_cores_within_numa =
        logical_CPUs_within_numa / num_logical_CPUs_within_core;
    num_numa_nodes = num_procs / logical_CPUs_within_numa;

    if (num_procs != (num_logical_CPUs_within_core * num_cores_within_numa *
                      num_numa_nodes)) {
      if (mpi.isMaster())
        print_with_prefix(
            "Error: Affinity feature does not support heterogeneous systems."
            "(num_procs=%d, SMT=%d, Cores=%d, NUMA Nodes=%d)\n",
            num_procs, num_logical_CPUs_within_core, num_cores_within_numa,
            num_numa_nodes);
    }
    ...
    for (int numa = 0; numa < num_numa_nodes; ++numa) {
      for (int core = 0; core < num_cores_within_numa; ++core) {
        for (int smt = 0; smt < num_logical_CPUs_within_core; ++smt) {
          int cpu_idx = ((numa * num_cores_within_numa) + core) *
                            num_logical_CPUs_within_core +
                        smt;
          assert(my_cpu_id(numa, core, smt) <= max_apic_id);
          apicid_to_cpu[my_cpu_id(numa, core, smt)] =
              (core_list[cpu_idx] & 0xFFFF);
        }
      }
    }
    ...
    if (mpi.isMaster()) {
      print_with_prefix(
          "CPU Topology: # of socket: %d, # of cores: %d, # of SMT: %d",
      ...
      /*
                              ...
                              for(int i = 0; i < num_procs; i++) {
                                      if((i % 100) == 0) fprintf(IMD_OUT,
         "\n%d-%d: ", i, i+100); fprintf(IMD_OUT, "%d,", numa_node_of_cpu(i));
                              }
                              fprintf(IMD_OUT, "\n]\n");
                              */
    }
    ...
    procs_per_numa_node =
        (numa_node_size + num_numa_nodes - 1) / num_numa_nodes;
    ...
    if (num_cores_within_numa != procs_per_numa_node * num_cores_assgined) {
      if (mpi.isMaster())
        print_with_prefix(
            "Warning: Core affinity is disabled because we cannot determine "
            "the # of cores to assign.");
  ...
  virtual int cpu(int tid) {
    ...
    int core = (num_cores_assgined * idx_within_numa_node) +
               (tid % num_cores_assgined);
  ...
  virtual int num_logical_CPUs() {
    return num_cores_assgined * num_logical_CPUs_within_core;
  }

  int cpu(int numa_rank, int core_rank, int smt_rank) {
    if (apicid_to_cpu == NULL || numa_rank >= num_numa_nodes ||
        core_rank >= num_cores_within_numa ||
        smt_rank >= num_logical_CPUs_within_core)
  ...
  int my_cpu_id(int numa_rank, int core_rank, int smt_rank) {
    return (((numa_rank << core_bits) | core_rank) << logical_CPU_bits) |
           smt_rank;
  }
...
void initialize_core_id() { my_apic_id = get_apic_id(); }

bool ensure_my_apic_id() {
  int cur_id = get_apic_id();
  if (my_apic_id != cur_id) {
    throw_exception(
        "Fatal error: Affinity is changed unexpectedly!!!(%d -> %d)\n",
        my_apic_id, cur_id);
  }
  return false;
}
...
void check_affinity_setting() {
  ...
  if (memcmp(&set, &initial_affinity, sizeof(set))) {
    ...
    print_with_prefix("th:%d affinity is changed [%s] -> [%s]", thread_id,
                      init_str.c_str(), now_str.c_str());
  }
}
...
void internal_set_core_affinity(int cpu) {
  ...
  print_with_prefix("th:%d thread started [%s](%d)", thread_id,
                    str_affinity.c_str(), cpu);
...
bool detect_core_affinity(std::vector<int>& cpu_set) {
  if (num_bfs_threads > num_omp_threads) {
    print_with_prefix(
        "BFS_NTHREADS must be equal or less than OMP_NUM_THREADS");
    return false;
  }
  // int total_threads = std::max(num_omp_threads, num_bfs_threads+1);
  int total_threads =
      std::max(num_omp_threads,
               num_bfs_threads);  // currently there are no background thread
...
#pragma omp parallel num_threads(num_omp_threads) \
  ...
  if (core_affinity && process_affinity) {
    print_with_prefix(
        "Error: failed to detect existing core binding. hetero ?");
    return false;
  }
  if (process_affinity) {
    ...
    if (core_binding != NULL) {
      ...
      do {
        ...
        core_id = (thread_id % core_binding->num_logical_CPUs());
               ...
               core_id >= num_bfs_threads + (is_extra_omp ? 1 : 0));
      if (core_affinity_enabled) {
        int cpu = core_binding->cpu(core_id);
        internal_set_core_affinity(cpu);
      }
...
void set_affinity() {
  ...
  if (num_node_str != NULL) {
    ...
    if (mpi.isMaster()) {
      print_with_prefix(
          "Warning: failed to get # of node (MPI_NUM_NODE=<# of node>). We "
          "assume 1 process per node");
    }
  }
  const char* dist_round_robin = getenv("MPI_ROUND_ROBIN");
  int max_procs_per_node = (mpi.size + num_node - 1) / num_node;
  int proc_rank = (dist_round_robin ? (mpi.rank / num_node)
                                    : (mpi.rank % max_procs_per_node));
  g_GpuIndex = proc_rank;

  if (mpi.isMaster()) {
    print_with_prefix("process distribution : %s",
  ...
  if (max_procs_per_node > 1 && max_procs_per_node != 3) {
    ...
    if (mpi.size_z > 1) {
      if (dist_round_robin) {
        MPI_Comm_split(MPI_COMM_WORLD, mpi.rank % num_node, mpi.rank_z,
                       &mpi.comm_z);
      } else {
        MPI_Comm_split(MPI_COMM_WORLD, mpi.rank / max_procs_per_node,
                       mpi.rank_z, &mpi.comm_z);
      }
      ...
      if (dist_round_robin == false && mpi.isRowMajor == false) {
        ...
        MPI_Comm_split(mpi.comm_2dc, mpi.rank_z, mpi.rank_2dc / mpi.size_z,
                       &mpi.comm_y);
      }
    }
  }
  ...
  if (affinity_mode == USE_EXISTING_AFFINITY) {
    ...
    if (detect_core_affinity(cpu_set) == false) {
      ...
      if (mpi.isMaster())
        print_with_prefix("Core affinity is enabled (using existing affinigy)");
    }
  }
...
#if NUMA_BIND
  if (core_binding == NULL) {
    AutoDetectCoreBinding* topology =
        new AutoDetectCoreBinding(max_procs_per_node, proc_rank);
      ...
      if (max_procs_per_node == 3) {
        if (numa_available() < 0) {
          ...
          if (NUM_SOCKET != topology->num_numa_nodes) {
            if (mpi.isMaster())
              print_with_prefix(
                  "Warning: # of NUMA nodes from the libnuma does not match "
                  "ours. (libnuma = %d, ours = %d)",
        ...
        if(proc_rank < topology->num_numa_nodes) {
                for(int core = 0; core < topology->num_cores_within_numa;
        core++) for(int smt = 0; smt < topology->num_cores_within_numa; smt++)
                                CPU_SET(topology->cpu(proc_rank, core, smt),
        &set);
        }
        else {
                for(int i = 0; i < topology->num_procs; i++)
        ...
        */
        ...
        if (mpi.isMaster()) { /* print from max rank node for easy debugging */
          print_with_prefix(
              "affinity for executing 3 processed per node is enabled.");
        }
      } else {
        if (numa_available() < 0) {
          print_with_prefix("No NUMA support available on this system.");
...
#endif  // #if NUMA_BIND
  if (mpi.isMaster()) {
    print_with_prefix("Y dimension is %s",
...
static void setup_2dcomm() {
  ...
  if (!success && tofu_6d) {
    int rank6d[6];
    int size6d[6];
    // FJMPI_Topology_rel_rank2xyzabc(mpi.rank, &rank6d[0], &rank6d[1],
    // &rank6d[2], &rank6d[3], &rank6d[4], &rank6d[5]);
    FJMPI_Topology_get_coords(MPI_COMM_WORLD, mpi.rank, FJMPI_TOFU_REL, 6,
                              rank6d);
    ...
    if (mpi.isMaster())
      print_with_prefix("Detected dimension %dx%dx%dx%dx%dx%d = %d", size6d[0],
                        size6d[1], size6d[2], size6d[3], size6d[4], size6d[5],
                        mpi.size);
    if (total != mpi.size) {
      ...
      if (ss_r.size() != 1 && ss_r.back() != 1 && ss_r.back() % 2 != 0) {
        if (mpi.isMaster())
          print_with_prefix(
              "Last dimension of R must be multiple of 2. But it is %d.\n",
              ss_r.back());
        MPI_Finalize();
        exit(1);
        ...
        if (mpi.isMaster())
          print_with_prefix(
              "Last dimension of C must be multiple of 2. But it is %d,\n",
              ss_c.back());
        MPI_Finalize();
        exit(1);
      }
      ...
      //		rank6d[0], rank6d[1], rank6d[2], rank6d[3], rank6d[4],
      // rank6d[5], mpi.rank_2dr, mpi.rank_2dc);

      success = true;
    }
  }

  const char* tofu_3d_div_y =
      getenv("TOFU_3D_DIV_Y");  // Number of divisions for Y axis. e.g. When
                                // TOFU_3D_DIV_Y=4, (R x C) = (Z*4, X*Y/4)
  if (!success && tofu_3d_div_y) {
    ...
    if (Y % div != 0) {
      ...
      exit(1);
               ...
               (Y / div) !=
                   1) {  // Y/div = 1 is OK because network topology is torus.
      if (mpi.isMaster())
    ...
    if (mpi.isMaster())
      print_with_prefix(
          "Dimension (R x C) = %d x %d is mapped to (X x Y x Z) = %d x %d x "
          "%d\n",
          mpi.size_2dr, mpi.size_2dc, X, Y, Z);

    int x = mpi.rank % X;
    int y = (mpi.rank % (X * Y)) / X;
    mpi.comm_c.rank = mpi.rank_2dr =
        mpi.rank / (X * Y) + (mpi.rank % (X * Y)) / (X * DY) * Z;
    if (x == 0 && y % DY == 0)
      mpi.comm_r.rank = mpi.rank_2dc = 0;
    else if (x == 0)
      mpi.comm_r.rank = mpi.rank_2dc = X * DY - (y % DY);
    else if ((y % DY) % 2 == 0)
      mpi.comm_r.rank = mpi.rank_2dc = (X - 1) * (y % DY) + x;
    else
      mpi.comm_r.rank = mpi.rank_2dc = (X - 1) * (y % DY) + (X - x);
  ...
  if (!success && virt_4d) {
    ...
    if (mpi.isMaster())
      print_with_prefix("Provided dimension (RXxRYxCXxCY) = %dx%dx%dx%d = %d",
                        RX, RY, CX, CY, mpi.size);
  ...
  if (!success) {
    ...
    if (twod_r_str) {
      ...
      if (twod_r == 0 || (twod_c * twod_r) != mpi.size) {
        if (mpi.isMaster())
          print_with_prefix("# of MPI processes(%d) cannot be divided by %d",
                            mpi.size, twod_r);
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
    } else {
      ...
      if (twod_c * twod_r != mpi.size) {
        if (mpi.isMaster())
          print_with_prefix(
              "Could not find the RxC combination for the # of processes(%d)",
              mpi.size);
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
    }
  ...
  if (mpi.isMaster())
    print_with_prefix("Dimension: (R x C) = (%dx%d)", mpi.size_2dr,
                      mpi.size_2dc);
  ...
  if (getenv("INVERT_RC")) {
    ...
    if (mpi.isMaster())
      print_with_prefix("Inverted: (%dx%d)", mpi.size_2dr, mpi.size_2dc);
  }
...
static void setup_2dcomm_on_3d() {
  ...
  if (treed_map_str) {
    ...
    if (mpi.isMaster())
      fprintf(IMD_OUT, "Dimension: (%dx%dx%dx%d) -> (%dx%d)\n", X, Y, Z1, Z2,
              mpi.size_2dr, mpi.size_2dc);
    if (mpi.size != mpi.size_2dr * mpi.size_2dc) {
      if (mpi.isMaster())
        fprintf(IMD_OUT, "Error: # of processes does not match\n");
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
...
void setup_globals(int argc, char** argv, int SCALE, int edgefactor) {
  ...
  if (mpi.isMaster()) {
    print_with_prefix("Graph500 Benchmark: SCALE: %d, edgefactor: %d %s", SCALE,
                      edgefactor,
#ifdef NDEBUG
                      ""
#else
...
#if PRINT_WITH_TIME
    char buf[200];
    strftime(buf, sizeof(buf), "%Y/%m/%d %A %H:%M:%S %Z",
             localtime(&global_clock.l.tv_sec));
  ...
  CUDA_CHECK(cudaMemcpyToSymbol("mpig", &mpig, sizeof(mpig), 0,
...
template <typename T>
int allgatherv(T* sendbuf, T* recvbuf, int sendcount, MPI_Comm comm,
               int comm_size) {
  ...
  MPI_Allgatherv(sendbuf, sendcount, MpiTypeOf<T>::type, recvbuf, recv_cnt,
                 recv_off, MpiTypeOf<T>::type, comm);
  return recv_off[comm_size];
}

template <typename T>
void alltoall(T* sendbuf, T* recvbuf, int sendcount, MPI_Comm comm) {
  MPI_Alltoall(sendbuf, sendcount, MpiTypeOf<T>::type, recvbuf, sendcount,
               MpiTypeOf<T>::type, comm);
}
 ...
 */
template <typename T>
T* alltoallv(T* sendbuf, int* sendcount, int* sendoffset, int* recvcount,
             int* recvoffset, MPI_Comm comm, int comm_size) {
  sendoffset[0] = 0;
  ...
  T* recv_data =
      static_cast<T*>(xMPI_Alloc_mem(recvoffset[comm_size] * sizeof(T)));
  MPI_Alltoallv(sendbuf, sendcount, sendoffset, MpiTypeOf<T>::type, recv_data,
                recvcount, recvoffset, MpiTypeOf<T>::type, comm);
  return recv_data;
}
...
class ParallelPartitioning {
 public:
  ParallelPartitioning(int num_partitions)
    ...
    thread_counts_ = static_cast<T*>(cache_aligned_xmalloc(
        buffer_width_ * (max_threads_ * 2 + 1) * sizeof(T)));
    thread_offsets_ = thread_counts_ + buffer_width_ * max_threads_;

    partition_size_ = static_cast<T*>(
        cache_aligned_xmalloc((num_partitions_ * 2 + 1) * sizeof(T)));
...
#pragma omp parallel for
      for (int r = 0; r < num_partitions_; ++r) {
        ...
        for (int t = 0; t < max_threads; ++t) {
          thread_offsets_[(t + 1) * width + r] =
              thread_offsets_[t * width + r] + thread_counts_[t * width + r];
        }
      }
      return T(0);
    } else {
...
#pragma omp parallel for
      for (int r = 0; r < num_partitions_; ++r) {
        ...
        assert(thread_offsets_[max_threads_ * width + r] ==
               partition_offsets_[r + 1]);
      }
  ...
  T* get_offsets() {
    return &thread_offsets_[buffer_width_ * omp_get_thread_num()];
  }
  ...
  bool check() const {
    ...
    for (int r = 0; r < num_partitions_; ++r) {
      assert(thread_offsets_[0 * width + r] ==
             partition_offsets_[r] + thread_counts_[0 * width + r]);
      for (int t = 1; t < max_threads_; ++t) {
        assert(thread_offsets_[t * width + r] ==
               thread_offsets_[(t - 1) * width + r] +
                   thread_counts_[t * width + r]);
      }
    }
#endif
    return true;
...
class ScatterContext {
 public:
  explicit ScatterContext(MPI_Comm comm)
    ...
    thread_counts_ = static_cast<int*>(cache_aligned_xmalloc(
        buffer_width_ * (max_threads_ * 2 + 1) * sizeof(int)));
    thread_offsets_ = thread_counts_ + buffer_width_ * max_threads_;

    send_counts_ = static_cast<int*>(
        cache_aligned_xmalloc((comm_size_ * 2 + 1) * 2 * sizeof(int)));
...
#pragma omp parallel for if (comm_size_ > 1000)
  ...
  T* scatter(T* send_data) {
    ...
    for (int r = 0; r < comm_size_; ++r) {
      assert(thread_offsets_[0 * width + r] ==
             send_offsets_[r] + thread_counts_[0 * width + r]);
      for (int t = 1; t < max_threads_; ++t) {
        assert(thread_offsets_[t * width + r] ==
               thread_offsets_[(t - 1) * width + r] +
                   thread_counts_[t * width + r]);
  ...
  T* gather(T* send_data) {
    T* recv_data =
        static_cast<T*>(xMPI_Alloc_mem(send_offsets_[comm_size_] * sizeof(T)));
    MPI_Alltoallv(send_data, recv_counts_, recv_offsets_, MpiTypeOf<T>::type,
                  recv_data, send_counts_, send_offsets_, MpiTypeOf<T>::type,
                  comm_);
    return recv_data;
  }
  ...
  void alltoallv(void* sendbuf, void* recvbuf, MPI_Datatype type,
    ...
    MPI_Alltoallv(sendbuf, send_counts_, send_offsets_, type, recvbuf,
                  recv_counts_, recv_offsets_, type, comm_);
  }
...
void scatter(const Mapping mapping, int data_count, MPI_Comm comm) {
  ...
  typename Mapping::send_type* restrict partitioned_data =
      static_cast<typename Mapping::send_type*>(cache_aligned_xmalloc(
          data_count * sizeof(typename Mapping::send_type)));
#pragma omp parallel
  {
...
void gather(const Mapping mapping, int data_count, MPI_Comm comm) {
  ...
  int* restrict local_indices =
      static_cast<int*>(cache_aligned_xmalloc(data_count * sizeof(int)));
  typename Mapping::send_type* restrict partitioned_data =
      static_cast<typename Mapping::send_type*>(cache_aligned_xmalloc(
          data_count * sizeof(typename Mapping::send_type)));
...
#pragma omp for schedule(static)
  ...
  typename Mapping::send_type* restrict reply_verts =
      scatter.scatter(partitioned_data);
  int recv_count = scatter.get_recv_count();
  ::free(partitioned_data);

  // make reply data
  typename Mapping::recv_type* restrict reply_data =
      static_cast<typename Mapping::recv_type*>(cache_aligned_xmalloc(
          recv_count * sizeof(typename Mapping::recv_type)));
#pragma omp parallel for
...
const char* minimum_type(int64_t max_value) {
  if (max_value <= (int64_t(1) << 7))
    return "int8_t";
  else if (max_value <= (int64_t(1) << 8))
    return "uint8_t";
  else if (max_value <= (int64_t(1) << 15))
    return "int16_t";
  else if (max_value <= (int64_t(1) << 16))
    return "uint16_t";
  else if (max_value <= (int64_t(1) << 31))
    return "int32_t";
  else if (max_value <= (int64_t(1) << 32))
    return "uint32_t";
  else
    return "int64_t";
}
...
int encode_gpu_compat_signed(const int64_t* input, int length,
                             uint8_t* output) {
  enum { MAX_CODE_LENGTH = MAX_CODE_LENGTH_64, SIMD_WIDTH = 32 };
  uint8_t tmp_buffer[SIMD_WIDTH][MAX_CODE_LENGTH];
  uint8_t code_length[SIMD_WIDTH];
  ...
  for (int i = 0; i < length; i += SIMD_WIDTH) {
    ...
    for (int k = 0; k < width; ++k) {
      ...
      uint64_t v =
          (v_raw < 0) ? ((uint64_t)(~v_raw) << 1) | 1 : ((uint64_t)v_raw << 1);
...
class BitmapEncoder {
 public:
  ...
  static int64_t calc_capacity_of_values(int64_t bitmap_size,
                                         int num_bits_per_word,
    ...
    int64_t min_data_bytes =
        num_bits / 8 - max_packet_size * omp_get_max_threads();
    double overhead_factor =
        1 + (double)packet_overhead / (double)packet_min_bytes;
  ...
  bool bitmap_to_stream(const BitmapF& bitmap, int64_t bitmap_size,
                        void* output, int64_t* data_size, int64_t max_size) {
...
#pragma omp parallel reduction(| : b_break)
    {
      ...
      for (int64_t i = i_start; i < i_end; ++i) {
        ...
        while (bmp_val != BitmapType(0)) {
          ...
          if (remain_packet_length <
              (b64 ? MAX_CODE_LENGTH_64 : MAX_CODE_LENGTH_32)) {
          ...
          int len;
          if (b64) {
            VARINT_ENCODE_MACRO_64(ptr, diff, len);
          } else {
            VARINT_ENCODE_MACRO_32(ptr, diff, len);
          }
          ptr += len;
          ++num_int;
...
#define TO_S(offset, length) (O_TO_S(offset) + L_TO_S(length))
    ...
    for (; i < num_packet; ++i) {
      ...
      if (O_TO_S(pk_head[i + 1].offset - pk_head[i].offset) -
              L_TO_S(pk_head[i].length) >
          32)
        break;
    }
    VERVOSE(print_with_prefix(
        "Move %ld length",
    ...
    for (; i < num_packet; ++i) {
      memmove(outbuf + TO_S(pk_head[i].offset, pk_head[i].length),
              ...
              (i + 1 < num_packet) ? L_TO_S(pk_head[i + 1].length)
                                   : num_packet * sizeof(PacketIndex));
      pk_head[i + 1].offset =
          pk_head[i].offset + L_TO_S(pk_head[i].length) / sizeof(uint32_t);
    }

    *(uint32_t*)outbuf = pk_head[num_packet].offset;
    return O_TO_S(pk_head[num_packet].offset) +
           num_packet * sizeof(PacketIndex);
#undef O_TO_S
#undef L_TO_S
#undef TO_S
...
void decode_stream(void* stream, int64_t data_size, Callback cb) {
  ...
  for (int i = 0; i < num_packets; ++i) {
    ...
    for (int c = 0; c < num_int; ++c) {
      ...
      if (b64) {
        int64_t v;
        VARINT_DECODE_MACRO_64(ptr, v, len);
        cb(v);
      } else {
        int32_t v;
        VARINT_DECODE_MACRO_32(ptr, v, len);
        cb(v);
      }
      ptr += len;
    }
    assert(ptr ==
           srcbuf + pk_head[i].length + pk_head[i].offset * sizeof(uint32_t));
  }
}
...
class Pool {
 public:
  ...
  virtual void free(T* buffer) { free_list_.push_back(buffer); }

  virtual void clear() { clear_(); }

  bool empty() const { return free_list_.size() == 0; }
 ...
 protected:
  ...
  virtual T* allocate_new() { return new (malloc(sizeof(T))) T(); }
...
//! Only get() and free() are thread-safe. The other functions are NOT
//! thread-safe.
template <typename T>
class ConcurrentPool : public Pool<T> {
 ...
 public:
  ConcurrentPool() : Pool<T>() { pthread_mutex_init(&thread_sync_, NULL); }
  virtual ~ConcurrentPool() { pthread_mutex_lock(&thread_sync_); }
...
class vector_w : public std::vector<T*> {
 ...
 public:
  ~vector_w() {
    for (typename super_::iterator it = this->begin(); it != this->end();
         ++it) {
      (*it)->~T();
      ::free(*it);
    }
...
class Store {
 public:
  ...
  void submit(T* value) {
    ...
    if (buffer_length_ == filled_length_) expand();
...
class ConcurrentStack {
 public:
  ConcurrentStack() { pthread_mutex_init(&thread_sync_, NULL); }

  ~ConcurrentStack() { pthread_mutex_destroy(&thread_sync_); }
...
#pragma omp critical(pis_submit_time)
    times_.push_back(
        TimeElement((double)span_micro / 1000000.0, content, number));
  }
...
#pragma omp critical(pis_submit_counter)
 ...
 private:
  ...
  void printTimeResult() {
    ...
    MPI_Reduce(dbl_times, sum_times, num_times, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(dbl_times, max_times, num_times, MPI_DOUBLE, MPI_MAX, 0,
               MPI_COMM_WORLD);

    if (mpi.isMaster()) {
      for (int i = 0; i < num_times; ++i) {
        fprintf(IMD_OUT, "Time of %s, %d, Avg, %f, Max, %f, (ms)\n",
                times_[i].content, times_[i].number,
                sum_times[i] / mpi.size_2d * 1000.0, max_times[i] * 1000.0);
      }
    }
  ...
  void printCountResult() {
    ...
    MPI_Reduce(dbl_times, sum_times, num_times, MPI_INT64_T, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(dbl_times, max_times, num_times, MPI_INT64_T, MPI_MAX, 0,
               MPI_COMM_WORLD);

    if (mpi.isMaster()) {
      for (int i = 0; i < num_times; ++i) {
        int64_t sum = sum_times[i], avg = sum_times[i] / mpi.size_2d,
                maximum = max_times[i];
        fprintf(IMD_OUT, "%s, %d, Sum, %ld, Avg, %ld, Max, %ld\n",
                counters_[i].content, counters_[i].number, sum, avg, maximum);
      }
    }
...
class TimeSpan {
 ...
 public:
  ...
  TimeSpan operator+(TimeSpan span) { return TimeSpan(span_ + span.span_); }
  TimeSpan operator-(TimeSpan span) { return TimeSpan(span_ - span.span_); }
  ...
  double getSpan() { return (double)span_ / 1000000.0; }
...
void buffered_print(const char* format, ...) {
  ...
  pbuf->length +=
      snprintf(pbuf->buffer + pbuf->length, sizeof(buf), "[r:%d,%f] %s\n",
