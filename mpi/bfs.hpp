...
class BfsBase {
 ...
 public:
  enum {
    ...
    // Since the type of bitmap entry is int32_t and 1 cache line is composed of
    // 32 bitmap entries, 32 is effective value.
  ...
  class QueuedVertexes {
   public:
    ...
    void append_nocheck(TwodVertex val) { v[length++] = val; }
  ...
  void construct(const int scale, const int edgefactor,
    ...
    // THREADS_PER_BLOCK * NUMBER_PACKING_EDGE_LISTS * mpi.size_2dc
  ...
  template <typename T>
  void get_shared_mem_pointer(void*& ptr, int64_t width, T** local_ptr,
                              T** orig_ptr) {
  ...
  void allocate_memory() {
     ...
     * - next queue: This is vertex list in the top-down phase, and <pred,
     * target> tuple list in the bottom-up phase.
     ...
     * 		required size: half_bitmap_width * BOTTOM_UP_BUFFER (for each
     * process) 2) to store next queue vertices in the bottom-up expand phase
     * 		required size: half_bitmap_width * 2 (for each process)
     * - shared visited:
     * - shared visited update (to store the information to update shared
     * visited)
     * - current queue extra memory (is allocated dynamically when the it is
     * required)
    ...
    if (mpi.isMaster())
      print_with_prefix("Allocating shared buffer: %f GB per node.",
                        to_giga(s * 2));

    top_down_comm_.max_num_rows =
        graph_.num_local_verts_ * 16 / PRM::TOP_DOWN_PENDING_WIDTH + 1000;
    top_down_comm_.tmp_rows = (TopDownRow*)cache_aligned_xmalloc(
        top_down_comm_.max_num_rows * 2 * sizeof(TopDownRow));  // for debug

    thread_local_buffer_ = (ThreadLocalBuffer**)cache_aligned_xmalloc(
        sizeof(thread_local_buffer_[0]) * max_threads);

    const int bottom_up_vertex_count_per_thread =
        (bitmap_width / BU_SUBSTEP + max_threads - 1) / max_threads * NBPE;
    const int packet_buffer_length = std::max(
        sizeof(LocalPacket) *
            max_comm_size,  // for the top-down and bottom-up list search
    ...
    for (int i = 0; i < max_threads; ++i) {
      ThreadLocalBuffer* tlb =
          (ThreadLocalBuffer*)((uint8_t*)buffer_.thread_local_ +
                               buffer_width * i);
      tlb->cur_buffer = NULL;
    ...
    // work_buf_size_ = half_bitmap_width * PRM::BOTTOM_UP_BUFFER *
    // sizeof(BitmapType);
    work_buf_size_ = std::max<int64_t>(
        bitmap_width * sizeof(BitmapType) * mpi.size_2dr /
            mpi.size_z,                             // space to receive NQ
    ...
    int64_t total_size_of_shared_memory =
        bitmap_width * 3 * sizeof(BitmapType) *
            mpi.size_z +               // new and old visited and buffer
        work_buf_size_ * mpi.size_z +  // work_buf_
        bitmap_width * sizeof(BitmapType) *
            max_comm_size +  // shared visited memory
        sizeof(memory::SpinBarrier) +
        sizeof(int) * shared_offset_length;
    VERVOSE(if (mpi.isMaster())
                print_with_prefix("Allocating shared memory: %f GB per node.",
                                  to_giga(total_size_of_shared_memory)));

    void* smem_ptr = buffer_.shared_memory_ =
        shared_malloc(total_size_of_shared_memory);

    get_shared_mem_pointer<BitmapType>(smem_ptr, bitmap_width,
                                       (BitmapType**)&new_visited_, NULL);
    get_shared_mem_pointer<BitmapType>(smem_ptr, bitmap_width,
                                       (BitmapType**)&old_visited_, NULL);
    get_shared_mem_pointer<BitmapType>(smem_ptr, bitmap_width,
                                       (BitmapType**)&visited_buffer_,
                                       (BitmapType**)&visited_buffer_orig_);
    get_shared_mem_pointer<int8_t>(smem_ptr, work_buf_size_,
                                   (int8_t**)&work_buf_,
                                   (int8_t**)&nq_recv_buf_);
    ...
    assert(smem_ptr ==
           (int8_t*)buffer_.shared_memory_ + total_size_of_shared_memory);

    bottom_up_substep_ = new MpiBottomUpSubstepComm(mpi.comm_2dr);
    bottom_up_substep_->register_memory(buffer_.shared_memory_,
                                        total_size_of_shared_memory);
  ...
  void deallocate_memory() {
    free(buffer_.thread_local_);
    buffer_.thread_local_ = NULL;
    shared_free(buffer_.shared_memory_);
    buffer_.shared_memory_ = NULL;
    free(thread_local_buffer_);
    thread_local_buffer_ = NULL;
...
#pragma omp for nowait
   ...
   public:
    ...
    void* get_next() {
      ...
      if (num_buffers_ <= idx) {
        ...
        if (max_num_buffers < idx) max_num_buffers = idx;
      }
  ...
  class CommHandlerBase : public AlltoallBufferHandler {
   public:
    ...
    CommHandlerBase(ThisType* this__)
        : this_(this__), pool_(&this__->a2a_comm_buf_) {}
    virtual ~CommHandlerBase() {}
    virtual void* get_buffer() { return this->pool_->get_next(); }
   ...
   public:
    ...
    virtual void received(void* buf, int offset, int length, int src) {
      ...
      if (this_->growing_or_shrinking_) {
        this->this_->top_down_receive<true>((uint32_t*)buf + offset, length,
                                            tmp_rows, &num_rows);
      } else {
        this->this_->top_down_receive<false>((uint32_t*)buf + offset, length,
                                             tmp_rows, &num_rows);
      }
    ...
    virtual void finish() {
      VERVOSE(if (mpi.isMaster()) print_with_prefix("num_rows= %d / %d",
  ...
  class BottomUpCommHandler : public CommHandlerBase<int64_t> {
   public:
    BottomUpCommHandler(ThisType* this__) : CommHandlerBase<int64_t>(this__) {}
  ...
  */
    ...
    if (root_r == mpi.rank_2dr) {
      ...
      if (root_owner == mpi.rank_2d) {
        ...
        const int64_t group_offset = reorder_get_group_offset_from_local_id(
            root_local, graph_.reorder_bits_);
...
#endif
        MPI_Bcast(&root_reordered, 1, MpiTypeOf<int64_t>::type, root_c,
                  mpi.comm_2dr);
#ifdef PROFILE_REGIONS
  ...
  void expand_visited_bitmap() {
    ...
    if (mpi.rank_z == 0 && mpi.comm_y != MPI_COMM_NULL) {
      ...
      // TODO: asymmetric size for z. (MPI_Allgather -> MPI_Allgatherv or
      // MpiCol::allgatherv ?)
      ...
      MPI_Allgather(bitmap, shared_bitmap_width, get_mpi_type(bitmap[0]),
                    recv_buffer, shared_bitmap_width, get_mpi_type(bitmap[0]),
                    mpi.comm_y);
#if VERVOSE_MODE
      g_expand_bitmap_comm +=
          shared_bitmap_width * mpi.size_y * sizeof(BitmapType);
#endif
#ifdef PROFILE_REGIONS
      timer_stop(current_expand);
#endif
  ...
  void expand_nq_bitmap() {
    ...
    timer_start(current_expand);
#endif
    MPI_Allgather(bitmap, bitmap_width, get_mpi_type(bitmap[0]), recv_buffer,
                  bitmap_width, get_mpi_type(bitmap[0]), mpi.comm_2dr);
#ifdef PROFILE_REGIONS
    timer_stop(current_expand);
#endif
#if VERVOSE_MODE
...
#endif
      s_.offset[0] =
          MpiCol::allgatherv((TwodVertex*)visited_buffer_orig_, nq_recv_buf_,
                             node_nq_size, mpi.comm_y, mpi.size_y);
#ifdef PROFILE_REGIONS
      timer_stop(current_expand);
#endif
  ...
  int top_down_make_nq_list(bool with_z, TwodVertex shifted_rc,
                            int bitmap_width, TwodVertex* outbuf) {
...
#pragma omp single
      {
        ...
        if (rank_z == 0) {
          ...
          assert(th_offset[node_threads] <=
                 int(bitmap_width * sizeof(BitmapType) / sizeof(TwodVertex) *
                     size_z));
        }
        if (with_z) s_.sync->barrier();
...
#pragma omp for schedule(static) nowait
  ...
  void top_down_expand_nq_list(TwodVertex* nq, int nq_size) {
    ...
    if (work_extra_buf_ != NULL) {
      free(work_extra_buf_);
      work_extra_buf_ = NULL;
    }
    TwodVertex* recv_buf =
        (TwodVertex*)((int64_t(cq_size_) * int64_t(sizeof(TwodVertex)) >
                       work_buf_size_)
                          ? (work_extra_buf_ = cache_aligned_xmalloc(
                                 cq_size_ * sizeof(TwodVertex)))
                          : work_buf_);
#ifdef PROFILE_REGIONS
    timer_start(current_expand);
#endif
    MPI_Allgatherv(nq, nq_size, MpiTypeOf<TwodVertex>::type, recv_buf,
                   recv_size, recv_off, MpiTypeOf<TwodVertex>::type,
                   mpi.comm_r.comm);
#ifdef PROFILE_REGIONS
    timer_stop(current_expand);
#endif
  ...
  void top_down_expand() {
    ...
    int nq_size =
        top_down_make_nq_list(false, shifted_c, bitmap_width, nq_list);
  ...
  int make_list_from_bitmap(bool with_z, TwodVertex shifted_rc, BitmapF bmp,
                            int bitmap_width, TwodVertex* outbuf) {
    int size_z = with_z ? mpi.size_z : 1;
    int rank_z = with_z ? mpi.rank_z : 0;
...
#pragma omp for schedule(static) nowait
  ...
  int bottom_up_make_nq_list(bool with_z, TwodVertex shifted_rc,
                             TwodVertex* outbuf) {
    ...
    if (bitmap_or_list_) {
      ...
      node_nq_size = make_list_from_bitmap(with_z, shifted_rc, NQBmp,
                                           bitmap_width, outbuf);
    } else {
      int size_z = with_z ? mpi.size_z : 1;
      int rank_z = with_z ? mpi.rank_z : 0;
      ...
      for (int i = 0; i < BU_SUBSTEP; ++i) {
        part_offset[rank_z * BU_SUBSTEP + 1 + i] =
            old_visited_list_size_[i] - new_visited_list_size_[i];
      }
      if (with_z) s_.sync->barrier();
      if (rank_z == 0) {
        part_offset[0] = 0;
        ...
        assert(part_offset[num_parts] <=
               int(bitmap_width * sizeof(BitmapType) / sizeof(TwodVertex) * 2));
      }
      if (with_z) s_.sync->barrier();
...
#pragma omp parallel if (mt) \
    reduction(+ : dbg_inc0, dbg_exc0, dbg_inc1, dbg_exc1)
#else
#pragma omp parallel if (mt)
#endif
      for (int i = 0; i < BU_SUBSTEP; ++i) {
        int max_threads =
            omp_get_num_threads();  // Place here because this region may be
                                    // executed sequential.
        ...
        int start, end, old_size = old_visited_list_size_[i],
                        new_size = new_visited_list_size_[i];
        get_partition(old_size, max_threads, tid, start, end);
        int new_vis_start =
            std::lower_bound(new_vis, new_vis + new_size, old_vis[start]) -
            new_vis;
  ...
  void bottom_up_expand_nq_list() {
    ...
    int node_nq_size = bottom_up_make_nq_list(
        mpi.isYdimAvailable(), TwodVertex(mpi.rank_2dr) << lgl,
        (TwodVertex*)visited_buffer_orig_);
...
#pragma omp parallel if (recv_nq_size > 1024 * 16)
    {
      ...
      int64_t begin, end;
      get_partition(recv_nq_size, nq_recv_buf_, get_msb_index(NBPE),
                    node_threads, tid, begin, end);
...
#pragma omp parallel for
  ...
  void top_down_send(int64_t tgt, int lgl, int r_mask,
    ...
    pk.data.t[pk.length++] =
        reorder_get_reorder_id_from_edge(tgt, (uint32_t(1) << lgl) - 1);
#else
  ...
  void top_down_send_large(
    ...
    for (int i = 0; i < mpi.size_2dr; ++i) {
      if (start >= end) break;
#ifdef SMALL_REORDER_BIT
      int s_dest = reorder_get_r_id_from_edge(edge_array[start], r_mask, lgl);
#else
      ...
      do {
#ifdef SMALL_REORDER_BIT
        int dest = reorder_get_r_id_from_edge(edge_array[next], r_mask, lgl);
#else
...
#pragma omp for
        for (int64_t word_idx = 0; word_idx < bitmap_size; ++word_idx) {
          ...
          while (bit_flags != BitmapType(0)) {
            ...
            int64_t group_offset = group_id << graph_.reorder_bits_;
        ...
        for (int64_t i = 0; i < int64_t(cq_size_); ++i) {
          ...
          if (row_bitmap_i & mask) {
            ...
            uint64_t group_offset =
                ((src.value & local_mask) / graph_.num_group_verts_)
                << graph_.reorder_bits_;
...
#undef ELSE
  ...
  void top_down_search() {
    ...
    td_comm_.run_with_ptr(
        VertexConverter((uint64_t(1) << graph_.local_bits_) - 1));
...
#pragma omp parallel for reduction( \
    ...
    //      printf("degree[%d] = %d\n", graph_.orig_vertexes_[v],
    //      (int)graph_.degree_[v]);
    //    }
    //  }
    ...
    MPI_Allreduce(MPI_IN_PLACE, tmp, 2, MpiTypeOf<int64_t>::type, MPI_SUM,
                  mpi.comm_2d);
...
#pragma omp parallel
  ...
  template <bool growing>
  void top_down_receive(uint32_t* stream, int length, TopDownRow* rows,
                        volatile int* num_rows) {
    TRACER(td_recv);
    PROF(profiling::TimeKeeper tk_all);
...
#ifdef SMALL_REORDER_BIT
                    uint64_t group_offset =
                        (tgt_local / graph_.num_group_verts_)
                        << graph_.reorder_bits_;
                    TwodVertex tgt_orig =
                        group_offset |
                        invert_map[graph_.bit_id_to_local_id(tgt_local)];
  ...
  void botto_up_print_stt(int64_t num_blocks, int64_t num_vertexes,
    ...
    int64_t sum_stt[2];
    int64_t max_stt[2];
    MPI_Reduce(send_stt, sum_stt, 2, MpiTypeOf<int64_t>::type, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(send_stt, max_stt, 2, MpiTypeOf<int64_t>::type, MPI_MAX, 0,
               MPI_COMM_WORLD);
    if (mpi.isMaster() && sum_stt[0] != 0) {
      print_with_prefix(
          "Bottom-Up using List. Total %f M Vertexes / %f M Blocks = %f Max %f "
          "%%+ Vertexes %f %%+ Blocks",
          to_mega(sum_stt[0]), to_mega(sum_stt[1]),
          to_mega(sum_stt[0]) / to_mega(sum_stt[1]),
    ...
    if (mpi.isMaster()) {
      ...
      print_with_prefix("Bottom-Up: %" PRId64 " vertexes found. Break down ...",
                        total_nq);
      for (int i = 0; i < count_length; ++i) {
        print_with_prefix("step %d / %d  %f M Vertexes ( %f %% )", i + 1,
                          count_length, to_mega(phase_recv[i]),
                          (double)phase_recv[i] / (double)total_nq * 100.0);
      }
    }
  }
  ...
  struct UnvisitedBitmapFunctor {
    BitmapType* visited;
    UnvisitedBitmapFunctor(BitmapType* visited__) : visited(visited__) {}
  ...
  void swap_visited_memory(bool prev_bitmap_or_list) {
    ...
    if (bitmap_or_list_) {  // bitmap ?
      ...
      // we copy the VIS to the working memory to avoid corrupting the VIS
      // bitmap. bottom_up_bitmap search function begins with work_buf_ that
      // contains current VIS bitmap.
      int64_t bitmap_width = get_bitmap_size_local();
      memory::copy_mt(work_buf_, new_visited_,
                      bitmap_width * sizeof(BitmapType));
    }
    ...
    if (!bitmap_or_list_) {       // list ?
      if (prev_bitmap_or_list) {  // previous level is performed with bitmap ?
        ...
        BitmapType* bmp_vis_p[4];
        get_visited_pointers(bmp_vis_p, 4, old_visited_, BU_SUBSTEP);
        TwodVertex* list_vis_p[4];
        get_visited_pointers(list_vis_p, 4, new_visited_, BU_SUBSTEP);
        // int64_t shifted_c = int64_t(mpi.rank_2dc) << graph_.lgl_;
        for (int i = 0; i < BU_SUBSTEP; ++i)
          new_visited_list_size_[i] = make_list_from_bitmap(
              false, 0, UnvisitedBitmapFunctor(bmp_vis_p[i]), step_bitmap_width,
              list_vis_p[i]);
        std::swap(new_visited_, old_visited_);
  ...
  int bottom_up_search_bitmap_process_step(BitmapType* phase_bitmap,
...
#pragma omp for nowait
    for (int64_t blk_idx = 0; blk_idx < int64_t(num_blks); ++blk_idx) {
      ...
      BitmapType* blk_row_bitmap =
          graph_.row_bitmap_ + phase_bmp_off + blk_bmp_off;
      ...
      int64_t length_start =
          graph_.blk_off[phase_blk_off + blk_idx].length_start;
      ...
      for (; num_active_rows > 0; ++c) {
        ...
        for (; i >= 3; i -= 4) {
          ...
          bool connected[4] = {
              shared_visited_[src[0] >> LOG_NBPE] &
                  (BitmapType(1) << (src[0] & NBPE_MASK)),
              shared_visited_[src[1] >> LOG_NBPE] &
                  (BitmapType(1) << (src[1] & NBPE_MASK)),
              shared_visited_[src[2] >> LOG_NBPE] &
                  (BitmapType(1) << (src[2] & NBPE_MASK)),
              shared_visited_[src[3] >> LOG_NBPE] &
                  (BitmapType(1) << (src[3] & NBPE_MASK)),
          };
          for (int s = 0; s < 4; ++s) {
            if (connected[s]) {  // TODO: use conditional branch
              // add to next queue
              int orig = cur_rows[s].orig;
              blk_bitmap[orig >> LOG_NBPE] |=
                  (BitmapType(1) << (orig & NBPE_MASK));
#if STREAM_UPDATE
              if (packet.length == LocalPacket::BOTTOM_UP_LENGTH) {
                visited_count += packet.length;
                PROF(profiling::TimeKeeper tk_commit);
        ...
        for (; i >= 0; --i) {
          ...
          if (shared_visited_[src >> LOG_NBPE] &
              (BitmapType(1) << (src & NBPE_MASK))) {
            // add to next queue
...
#pragma omp barrier
  ...
  TwodVertex bottom_up_search_list_process_step(
      ...
      int64_t& num_blocks,
#endif
      TwodVertex* phase_list, TwodVertex phase_size, int8_t* vertex_enabled,
      TwodVertex* write_list, TwodVertex phase_bmp_off,
      TwodVertex half_bitmap_width, int* th_offset) {
    TRACER(bu_list_step);
    int max_threads = omp_get_num_threads();
    int target_rank = phase_bmp_off / half_bitmap_width / 2;
    ...
    int64_t begin, end;
    get_partition(phase_size, phase_list, LOG_BFELL_SORT, max_threads, tid,
                  begin, end);
...
#pragma omp barrier
  ...
  void bottom_up_search_bitmap_process_block(
      BitmapType* __restrict__ phase_bitmap, int off_start, int off_end,
      int phase_bmp_off, LocalPacket* buffer) {
    VERVOSE(int tmp_edge_relax = 0);
    ...
    int r_bits = graph_.r_bits_;
#ifdef SMALL_REORDER_BIT
    int reorder_bits = graph_.reorder_bits_;
    int group_bits = orig_lgl - reorder_bits;
    uint64_t local_mask = (int64_t(1) << lgl) - 1;
    ...
    for (int64_t blk_bmp_off = off_start; blk_bmp_off < off_end;
      ...
      while (bit_flags != BitmapType(0)) {
        ...
        int64_t tgt_group_offset = tgt_group_id << reorder_bits;
        ...
        uint64_t src_r = reorder_get_r_id_from_edge(src, r_mask, lgl);
        uint64_t src_local = reorder_get_reorder_id_from_edge(src, local_mask);
        ...
        if (shared_visited[bit_idx >> PRM::LOG_NBPE] &
          ...
          uint64_t src_orig =
              ((src & (~local_mask)) >> (lgl - group_bits)) | (src_group_id);
        ...
        for (auto e = e_start; e < e_end; ++e) {
          ...
          uint64_t src_r = reorder_get_r_id_from_edge(src, r_mask, lgl);
          uint64_t src_local =
              reorder_get_reorder_id_from_edge(src, local_mask);
    ...
    for (int64_t blk_bmp_off = off_start; blk_bmp_off < off_end;
      ...
      while (bit_flags != BitmapType(0)) {
        ...
        TwodVertex bit_idx =
            (separated_src >> lgl) * L + (separated_src & lmask);
        if (shared_visited[bit_idx >> PRM::LOG_NBPE] &
            (BitmapType(1) << (bit_idx & PRM::NBPE_MASK))) {
          // add to next queue
          visited_i |= vis_bit;
          buffer->data.b[num_send + 0] = separated_src;
          buffer->data.b[num_send + 1] =
              (phase_bmp_off + blk_bmp_off) * PRM::NBPE + idx;
          num_send += 2;
        }
      }  // while(bit_flags != BitmapType(0)) {
      // write back
  ...
  // returns the number of vertices found in this step.
  int bottom_up_search_bitmap_process_step(BottomUpSubstepData& data,
...
#pragma omp barrier
    ...
    while (true) {
      ...
      bottom_up_search_bitmap_process_block(phase_bitmap, off_start, off_end,
                                            phase_bmp_off, buffer);
...
#pragma omp barrier
  ...
  TwodVertex bottom_up_search_list_process_step(
      ...
      int64_t& num_blocks,
#endif
      BottomUpSubstepData& data, int target_rank, int8_t* vertex_enabled,
      TwodVertex* write_list, TwodVertex step_bitmap_width, int* th_offset) {
    TRACER(bu_list_step);
    int max_threads = omp_get_num_threads();
    ...
    int64_t begin, end;
    get_partition(data.tag.length, phase_list, LOG_BFELL_SORT, max_threads, tid,
                  begin, end);
    ...
    for (int i = begin; i < end;) {
      ...
      do {
        ...
        if (row_bitmap_i & vis_bit) {  // I have edges for this vertex ?
          ...
          int64_t group_offset = group_id << reorder_bits;
...
#pragma omp barrier
  ...
  void bottom_up_gather_nq_size(int64_t* visited_count) {
    ...
    MPI_Allreduce(&nq_size, &global_nq_size_, 1, MpiTypeOf<int64_t>::type,
                  MPI_SUM, mpi.comm_2d);
#else
    ...
    MPI_Reduce(visited_count, red_nq_size, mpi.size_2dc, MPI_INT, MPI_SUM, 0,
               mpi.comm_2dr);
    if (mpi.rank_2dr == 0) {
      ...
      MPI_Allreduce(&sum_nq_size, &global_nq_size, 1, MpiTypeOf<int64_t>::type,
                    MPI_SUM, mpi.comm_2dc);
      for (int i = 0; i < mpi.size_2dc; ++i) {
    ...
    MPI_Scatter(scatter_buffer, sizeof(recv_nq_size), MPI_BYTE, &recv_nq_size,
                sizeof(recv_nq_size), MPI_BYTE, 0, mpi.comm_2dr);
  ...
  void bottom_up_bmp_parallel_section(int64_t* visited_count) {
    ...
    int buffer_count =
        work_buf_size_ / (step_bitmap_width * sizeof(BitmapType));
    ...
    bottom_up_substep_->begin(bitmap_buffer + BU_SUBSTEP,
                              buffer_count - BU_SUBSTEP, step_bitmap_width);
...
#pragma omp barrier
  ...
  void bottom_up_list_parallel_section(int64_t* visited_count,
    ...
    assert(work_buf_size_ >= bitmap_width * PRM::BOTTOM_UP_BUFFER);
    int buffer_size =
        step_bitmap_width * sizeof(BitmapType) / sizeof(TwodVertex);
    int buffer_count =
        work_buf_size_ / (step_bitmap_width * sizeof(BitmapType));
    ...
    bottom_up_substep_->begin(list_buffer + BU_SUBSTEP + 1,
                              buffer_count - BU_SUBSTEP - 1, buffer_size);
...
#pragma omp barrier
          ...
          int new_visited_cnt = bottom_up_search_list_process_step(
              ...
              num_blocks,
#endif
              data, target_rank, vertex_enabled, write_buffer,
              step_bitmap_width, th_offset);
...
#pragma omp barrier
  ...
  /*
          struct BottomUpListParallelSection : public Runnable {
                  ...
                  BottomUpListParallelSection(ThisType* this__, int*
     visited_count_, int8_t* vertex_enabled_) : this_(this__),
     visited_count(visited_count_) , vertex_enabled(vertex_enabled_) ,
     num_blocks(0), num_vertexes(0) { } virtual void run() { this_->b;
                  }
          };
  */
  void bottom_up_search_list() {
    ...
    int buffer_size =
        half_bitmap_width * sizeof(BitmapType) / sizeof(TwodVertex);
    ...
    int64_t num_blocks;
    int64_t num_vertexes;
    bottom_up_list_parallel_section(visited_count, vertex_enabled, num_blocks,
                                    num_vertexes);
    bu_comm_.run();
    bottom_up_gather_nq_size(visited_count);
  ...
  struct BottomUpReceiver : public Runnable {
    ...
    virtual void run() {
      ...
      int reorder_bits = this_->graph_.reorder_bits_;
      int group_bits = orig_lgl - reorder_bits;
      uint64_t orig_lmask = (uint64_t(1) << orig_lgl) - 1;
#else
      int orig_lgl = this_->graph_.orig_local_bits_;
      ...
      for (int i = 0; i < length; ++i) {
        ...
        int64_t dst_id =
            ((pred_dst & ((uint64_t(1) << group_bits) - 1)) << reorder_bits) |
  ...
  static void printInformation(int validation_level, bool pre_exec,
                               bool real_benchmark) {
...
#undef PRINT_VAL
  ...
  TwodVertex*
      nq_recv_buf_;  // shared memory (memory space is shared with work_buf_)
...
void BfsBase::run_bfs_core(int64_t root, int64_t* pred, const int edgefactor,
  ...
  g_tp_comm = g_bu_pred_comm = g_bu_bitmap_comm = g_bu_list_comm =
      g_expand_bitmap_comm = g_expand_list_comm = 0;
...
#if VERVOSE_MODE
  if (mpi.isMaster())
    print_with_prefix("Time of initialize memory: %f ms",
  ...
  char* prof_mes[] = {const_cast<char*>("bottom-up"),
                      const_cast<char*>("top-down")};
#endif
  ...
  while (true) {
    ...
    prev_time = tmp;
    AsyncAlltoallManager* a2a_comm =
        forward_or_backward_ ? &td_comm_ : &bu_comm_;
    ...
    if (forward_or_backward_) {
      recv_proc_thread_large_time_.submit("recv proc thread large",
                                          current_level_);
    }
    ...
    if (forward_or_backward_) {
      profiling::g_pis.submitCounter(num_edge_top_down_, "top-down edge relax",
                                     current_level_);
      profiling::g_pis.submitCounter(num_td_large_edge_, "top-down large edge",
                                     current_level_);
    } else
      profiling::g_pis.submitCounter(num_edge_bottom_up_,
                                     "bottom-up edge relax", current_level_);

    int64_t send_num_edges[] = {num_edge_top_down_, num_td_large_edge_,
                                num_edge_bottom_up_};
    int64_t recv_num_edges[3];
    MPI_Reduce(send_num_edges, recv_num_edges, 3, MpiTypeOf<int64_t>::type,
               MPI_SUM, 0, MPI_COMM_WORLD);
...
#endif
#if !VERVOSE_MODE
    if (global_nq_size_ == 0) break;
#endif
#if ENABLE_FUJI_PROF
    fapp_start("expand", 0, 0);
#endif
    next_bitmap_or_list = !forward_or_backward_;
    if (growing_or_shrinking_ &&
        global_nq_size_ > prev_global_nq_size) {  // growing
      if (forward_or_backward_) {                 // forward ?
        if (global_nq_edges_ >
            int64_t(graph_.num_global_edges_ / alpha)) {  // switch to backward
      ...
      if (!forward_or_backward_) {  // backward ?
        if (global_nq_size_ <
            int64_t(graph_.num_global_verts_ /
                    (beta * edgefactor * 2.0))) {  // switch to topdown
          ...
          // int threashold = std::min<int>(max_capacity,
          // bitmap_width*sizeof(BitmapType)/2);
          int bitmap_width = get_bitmap_size_local();
          double threashold = bitmap_width * sizeof(BitmapType) /
                              sizeof(TwodVertex) / denom_bitmap_to_list_;
    ...
    if (mpi.isMaster()) {
      ...
      print_with_prefix("NQ %" PRId64
                        ", 1/ %f, %f %% of global, 1/ %f, %f %% of Unvisited",
                        global_nq_size_, 1 / nq_rate, nq_rate * 100,
                        1 / nq_unvis_rate, nq_unvis_rate * 100);
      print_with_prefix("Unvisited %" PRId64 ", 1/ %f, %f %% of global",
                        global_unvisited_vertices, 1 / unvis_rate,
                        unvis_rate * 100);

      int64_t edge_relaxed =
          forward_or_backward_ ? num_edge_top_down_ : num_edge_bottom_up_;
      print_with_prefix(
          "Edge relax: %" PRId64 ", %f %%, %f M/s (Level), %f M/s (Fold)",
      ...
      print_with_prefix("Queue buffer: %f MB per node, Max %f %%+",
                        to_mega(total_qb_size / mpi.size_2d),
                        diff_percent(max_qb_size, total_qb_size, mpi.size_2d));
...
#if VERVOSE_MODE
  if (mpi.isMaster())
    print_with_prefix("Time of BFS: %f ms",
                      (MPI_Wtime() - start_time) * 1000.0);
  int64_t total_edge_relax = total_edge_top_down + total_edge_bottom_up;
  int time_cnt = 2, cnt_cnt = 9;
  double send_time[] = {fold_time, expand_time}, sum_time[time_cnt],
         max_time[time_cnt];
  int64_t send_cnt[] = {
      g_tp_comm,           g_bu_pred_comm,       g_bu_bitmap_comm,
  ...
  MPI_Reduce(send_time, sum_time, time_cnt, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(send_time, max_time, time_cnt, MPI_DOUBLE, MPI_MAX, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(send_cnt, sum_cnt, cnt_cnt, MpiTypeOf<int64_t>::type, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(send_cnt, max_cnt, cnt_cnt, MpiTypeOf<int64_t>::type, MPI_MAX, 0,
             MPI_COMM_WORLD);
  if (mpi.isMaster()) {
    ...
    printCounter("Avg bottom-up pred update recv: %f MiB, %f %%+", sum_cnt,
                 max_cnt, 1);
    printCounter("Avg bottom-up bitmap send: %f MiB, %f %%+", sum_cnt, max_cnt,
                 2);
    printCounter("Avg bottom-up list send: %f MiB, %f %%+", sum_cnt, max_cnt,
                 3);
    printCounter("Avg expand bitmap recv: %f MiB, %f %%+", sum_cnt, max_cnt, 4);
    printCounter("Avg expand list recv: %f MiB, %f %%+", sum_cnt, max_cnt, 5);
    printCounter("Avg top-down traversed edges: %f MiB, %f %%+", sum_cnt,
                 max_cnt, 6);
    printCounter("Avg bottom-up traversed edges: %f MiB, %f %%+", sum_cnt,
                 max_cnt, 7);
    printCounter("Avg total relaxed traversed: %f MiB, %f %%+", sum_cnt,
                 max_cnt, 8);
  }
#endif
}
