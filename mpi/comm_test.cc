...
struct TestBuffer : public CommunicationBuffer {
  ...
  virtual void* base_object() { return this; }
  virtual int element_size() { return sizeof(int); }
  virtual void* pointer() { return mem; }
};
...
struct TestBufferHandler : public AlltoallBufferHandler {
  ...
  virtual MPI_Datatype data_type() { return MPI_INT; }
  virtual void finished() { fiber_->end_processing(); }
  virtual CommunicationBuffer* alloc_buffer() { return this->pool_->get(); }
  virtual void received(CommunicationBuffer* buf_, int src) {
    fiber_->submit(
        new DataChecker(static_cast<TestBuffer*>(buf_->base_object()), pool_),
        1);
  }
  FiberManager* fiber_;
  memory::Pool<TestBuffer>* pool_;
...
int main(int argc, char** argv) {
...

  {
    ...
    if (mpi.isMaster())
      printf("[%f] complete initialization\n", MPI_Wtime() - start);
