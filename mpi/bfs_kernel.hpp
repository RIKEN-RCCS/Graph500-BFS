...
// Initialize
void cu_initialize_memory(int num_local_vertices, int bitmap_size_visited,
                          int bitmap_size_v0, int bitmap_size_v1,
                          int summary_size, int64_t* pred, uint32_t* nq_bitmap,
                          uint32_t* nq_sorted_bitmap, uint32_t* visited,
                          uint32_t* cq_summary, uint32_t* cq_bitmap,
                          uint32_t* shared_visited, cudaStream_t* streams,
                          int num_streams) {
  memset_gpu<int64_t>(pred, -1, num_local_vertices, streams[0 % num_streams]);
  memset_gpu<uint32_t>(nq_bitmap, 0, bitmap_size_visited,
                       streams[1 % num_streams]);
  memset_gpu<uint32_t>(nq_sorted_bitmap, 0, bitmap_size_visited,
                       streams[2 % num_streams]);
  memset_gpu<uint32_t>(visited, 0, bitmap_size_visited,
                       streams[3 % num_streams]);
  memset_gpu<uint32_t>(shared_visited, 0, bitmap_size_v1,
                       streams[4 % num_streams]);
  if (cq_summary && cq_bitmap) {
    memset_gpu<uint32_t>(cq_summary, 0, summary_size, streams[5 % num_streams]);
    memset_gpu<uint32_t>(cq_bitmap, 0, bitmap_size_v0,
                         streams[6 % num_streams]);
  }
  CUDA_CHECK(cudaThreadSynchronize());
}

__global__ void initialize_create_column_list(int summary_size,
...
__global__ void create_column_list_count_kernel(
    uint32_t* __restrict__ const bitmap, uint32_t* __restrict__ const summary,
    int summary_size, int* __restrict__ const count) {
  ...
  for (uint i = 0; i < sizeof(i_summary) * 8; ++i) {
    if (i_summary & ((uint32_t)1 << i)) {
      const uint idx =
          (WARP_IDX * sizeof(i_summary) * 8 + i) * WARP_SIZE + threadIdx.x;
  ...
  // We don't need to synchronize because the next reduction is performed within
  // a warp.
  //	__syncthreads();
...
__global__ void create_column_list_set_kernel(
    ...
    volatile uint32_t* __restrict__ const summary, int summary_size,
    int* __restrict__ const offset, uint2* __restrict__ const columns) {
  if (WARP_IDX >= summary_size) return;
  ...
  for (int i = 0; i < sizeof(uint32_t) * 8; ++i) {
    if (summary_flags & ((uint32_t)1 << i)) {
      const int idx =
          (WARP_IDX * sizeof(uint32_t) * 8 + i) * WARP_SIZE + threadIdx.x;
      const uint32_t i_bitmap = bitmap[idx];
      // clear CQ
      bitmap[idx] = 0;
      const uint cq_flags = __ballot(i_bitmap);
...
__global__ void fill_cq_list_last_block(int c, const int* const offset,
                                        uint2* const columns) {
...
void create_cq_list(uint32_t* bitmap, uint32_t* summary, int summary_size,
                    PrefixSumGPU<int>* cq_count, uint2* column_buffer,
                    int empty_column, cudaStream_t stream) {
  int nblocks;
  ...
  create_column_list_count_kernel<<<nblocks, dim3(WARP_SIZE, WARPS_PER_BLOCK),
                                    0, stream>>>(bitmap, summary, summary_size,
                                                 cq_count->get_buffer() + 1);

  (*cq_count)(&stream, 1);

  create_column_list_set_kernel<<<nblocks, dim3(WARP_SIZE, WARPS_PER_BLOCK), 0,
                                  stream>>>(
...
__device__ int read_graph_is_visited(
    uint32_t* __restrict__ const shared_visited, int64_t e1) {
...
__global__ void initialize_read_graph_input(ReadGraphInput* input) {
...
__global__ void initialize_filter_edges_input(ReadGraphInput* input,
                                              int num_edges) {
...
__global__ void read_graph_and_store(
    ReadGraphInput* in_param, const int64_t* row_starts,
    const int32_t* index_array_high, const uint16_t* index_array_low,
    uint2* columns, const int column_start, const int column_end,
    const int empty_column, int* out_columns, int64_t* out_indices) {
...

  {
    const int column_offset =
        column_start + THREADS_PER_BLOCK * blockIdx.x + tid;
  ...
  if (tid == 0) {
    ...
    if (total_length == 0) {
      ...
      s_skip_task = true;
    } else if (atomicAdd(&in_param->reserve_offset, total_length) +
                   total_length >
      ...
      if (tid < remain) {
        ...
        if ((bitmap & (1U << v0_lowbits)) == 0) {
          ...
          printf("K:%d:B(%d)T(%d,%d)c_value(%d) is out of range\n", __LINE__,
                 blockIdx.x, threadIdx.x, threadIdx.y, c_value);
        }
#endif
#if 0
...
// blockDim: dim3(WARP_SIZE, WARPS_PER_BLOCK)
__global__ void read_edges_and_filter(ReadGraphInput* in_param,
                                      const int* columns,
  ...
  const int i_end =
      min(i_start + THREADS_PER_BLOCK * LOOPS_PER_THREAD, num_in_edges);
  ...
  for (int i = i_start; i < i_end; i += THREADS_PER_BLOCK) {
    ...
    if (tid < WARPS_PER_BLOCK)
      dev_simd_kogge_stone<int, LOG_WARPS_PER_BLOCK>(s_sum, tid);
    ...
    if (valid) {
      ...
      const int out_offset =
          out_base + s_offset[threadIdx.y] + __popc(mask & valid_flags);
#if KD_PRINT
      if (out_offset < 0 ||
          out_offset >= out_base + s_offset[WARPS_PER_BLOCK]) {
        printf(
            "K:%d:B(%d)T(%d,%d)Error:out_offset(%d),out_base(%d),warp_base(%d),"
            "my_offset(%d),total_length(%d),.\n",
            __LINE__, blockIdx.x, threadIdx.x, threadIdx.y, out_offset,
            out_base, s_offset[threadIdx.y], __popc(mask & valid_flags),
            s_offset[WARPS_PER_BLOCK]);
      }
#endif
#if 0
			if(v0 >= (1 << 16)) {
...
void read_graph_1(ReadGraphInput* in_param, const int64_t* row_starts,
                  const int32_t* index_array_high,
                  const uint16_t* index_array_low, uint2* columns,
                  const int column_start, const int column_end,
                  const int empty_column, uint32_t* shared_visited,
                  int* out_columns, int64_t* out_indices, long2* out_edges,
                  cudaStream_t stream) {
  initialize_read_graph_input<<<1, 1, 0, stream>>>(in_param);

  int num_ctas = nblocks2n<int>(column_end - column_start, THREADS_PER_BLOCK);
  read_graph_and_store<<<num_ctas, dim3(WARP_SIZE, WARPS_PER_BLOCK), 0,
                         stream>>>(
      in_param, row_starts, index_array_high, index_array_low, columns,
      column_start, column_end, empty_column, out_columns, out_indices);
}

void read_graph_2(ReadGraphInput* in_param, const int64_t* row_starts,
                  const int32_t* index_array_high,
                  const uint16_t* index_array_low, uint2* columns,
                  const int column_start, const int column_end,
                  const int empty_column, uint32_t* shared_visited,
                  int* out_columns, int64_t* out_indices, long2* out_edges,
                  cudaStream_t stream) {
  int num_ctas = nblocks2n<int>(READ_GRAPH_OUTBUF_SIZE,
                                THREADS_PER_BLOCK * LOOPS_PER_THREAD);
  read_edges_and_filter<<<num_ctas, dim3(WARP_SIZE, WARPS_PER_BLOCK), 0,
                          stream>>>(in_param, out_columns, out_indices,
                                    shared_visited, out_edges);
}

void filter_edges(ReadGraphInput* in_param, int num_edges,
                  const int* in_columns, const int64_t* in_indices,
                  uint32_t* shared_visited, long2* out_edges,
                  cudaStream_t stream) {
  initialize_filter_edges_input<<<1, 1, 0, stream>>>(in_param, num_edges);

  int num_ctas =
      nblocks2n<int>(num_edges, THREADS_PER_BLOCK * LOOPS_PER_THREAD);
  read_edges_and_filter<<<num_ctas, dim3(WARP_SIZE, WARPS_PER_BLOCK), 0,
                          stream>>>(in_param, in_columns, in_indices,
                                    shared_visited, out_edges);
}
...
__global__ void decode_varint_kernel(
    ...
    const uint8_t* stream, const int num_packet, T* output) {
  ...
  decode_varint_packet(stream + stream_offset, num_int, output + v_offset,
...
__global__ void decode_varint_signed_kernel(
  ...
  decode_varint_packet_signed(stream + stream_offset, num_int,
                              output + v_offset, *s_offset, threadIdx.x);
}

template <typename T>
void decode_varint_stream(
    const int4* packet_list,  // (stream_offset, num_int, v_offset, *)
    const uint8_t* byte_stream, const int num_packet, T* output,
    cudaStream_t stream) {
  int num_ctas = nblocks2n<int>(num_packet, WARPS_PER_BLOCK);
  if (num_ctas == 0) return;
  decode_varint_kernel<T>
      <<<num_ctas, dim3(WARP_SIZE, WARPS_PER_BLOCK), 0, stream>>>(
...
template void decode_varint_stream_signed<int32_t>(
    const int4* packet_list,  // (stream_offset, num_int, v_offset, *)
    const uint8_t* byte_stream, const int num_packet, int32_t* output,
    cudaStream_t stream);
template void decode_varint_stream_signed<int64_t>(
    const int4* packet_list,  // (stream_offset, num_int, v_offset, *)
    const uint8_t* byte_stream, const int num_packet, int64_t* output,
    cudaStream_t stream);

// blockDim: (PACKET_LENGTH/2, THREADS_PER_BLOCK*2/PACKET_LENGTH)
__global__ void receiver_processing_kernel(
    const int4* const
        input_packets,  // (stream_offset, num_edges, v1_offset, stream_length)
    const int num_packets, const int64_t* const v0_list,
    const uint32_t* const v1_list, uint32_t* const nq_bitmap,
    uint32_t* const nq_sorted_bitmap, uint32_t* const visited,
    int64_t* const pred, const uint32_t* const v1_map,

    // for parent vertex conversion
    const int log_local_verts, const int64_t log_size,
    const int64_t local_verts_mask, const int current_level,
...
#endif
  if (HARF_PACKET * 0 + tidx < num_edges)
    s_v0[HARF_PACKET * 0 + tidx] = v0_start[HARF_PACKET * 0 + tidx];
  if (HARF_PACKET * 1 + tidx < num_edges)
    s_v0[HARF_PACKET * 1 + tidx] = v0_start[HARF_PACKET * 1 + tidx];

  __syncthreads();

#if KD_PRINT
  if (HARF_PACKET * 0 + tidx < num_edges && s_v0[tidx] >= (int64_t(1) << 28)) {
    printf("K:%d:R(%d)B(%d)T(%d,%d)Error:  s_v0[tidx] (%" PRId64
           ") is too big!!!\n",
           __LINE__, mpig.rank_2d, blockIdx.x, threadIdx.x, threadIdx.y,
           s_v0[tidx]);
  }
  if (HARF_PACKET * 1 + tidx < num_edges &&
      s_v0[tidx + HARF_PACKET] >= (int64_t(1) << 28)) {
    printf("K:%d:R(%d)B(%d)T(%d,%d)Error:  s_v0[tidx + HARF_PACKET] (%" PRId64
           ") is too big!!!\n",
           __LINE__, mpig.rank_2d, blockIdx.x, threadIdx.x, threadIdx.y,
    ...
    if ((visited[word_idx] & mask) == 0) {
      if ((atomicOr(&visited[word_idx], mask) & mask) == 0) {
        ...
        if ((pred[v1_orig] & 0xFFFFFFFFFFFF) >= (int64_t(1) << 28)) {
          ...
          printf("K:%d:B(%d)T(%d,%d):DumpInfo:v0_swizzled(%" PRId64
                 "),log_local_verts(%d),local_verts_mask(%" PRId64
                 "),log_size(%d),current_level(%d)\n",
                 __LINE__, blockIdx.x, threadIdx.x, threadIdx.y, v0_swizzled,
                 log_local_verts, local_verts_mask, log_size, current_level);
        }
#endif
#if 0
...
void receiver_processing(
    const int4*
        input_packets,  // (stream_offset, num_edges, v1_offset, stream_length)
    int num_packets, const int64_t* v0_list, const uint32_t* v1_list,
    uint32_t* nq_bitmap, uint32_t* nq_sorted_bitmap, uint32_t* visited,
    int64_t* pred, const uint32_t* v1_map, int log_local_verts,
    int64_t log_size, int64_t local_verts_mask, int current_level,
    int* nq_count_ptr, cudaStream_t stream) {
  const dim3 blkDim(BFS_PARAMS::PACKET_LENGTH / 2,
                    THREADS_PER_BLOCK * 2 / BFS_PARAMS::PACKET_LENGTH);
  ...
  receiver_processing_kernel<<<num_ctas, blkDim, 0, stream>>>(
      ...
      visited, pred, v1_map, log_local_verts, log_size, local_verts_mask,
      current_level, nq_count_ptr);
}

void cu_clear_nq(int bitmap_size_visited, uint32_t* nq_bitmap,
                 uint32_t* nq_sorted_bitmap, cudaStream_t* streams,
                 int num_streams) {
  memset_gpu<uint32_t>(nq_bitmap, 0, bitmap_size_visited,
...
__global__ void update_bitmap_kernel_with_summary(
    const int4*
        input_packets,  // (stream_offset, num_vertices, v_offset, src_num)
    int num_packets, const uint32_t* v_list, uint32_t* bitmap,
    uint32_t* summary, int log_src_factor) {
  enum {
    HARF_PACKET = BFS_PARAMS::PACKET_LENGTH / 2,
  };
  ...
  if (HARF_PACKET * 0 + tidx < num_vertices)
    s_v0[HARF_PACKET * 0 + tidx] = v_start[HARF_PACKET * 0 + tidx];
  if (HARF_PACKET * 1 + tidx < num_vertices)
    s_v0[HARF_PACKET * 1 + tidx] = v_start[HARF_PACKET * 1 + tidx];
...
// num_packet: multiply of THREADS_PER_BLOCK*2/PACKET_LENGTH
void update_bitmap(const int4* input_packets,  // (stream_offset, num_vertices,
                                               // src_num, stream_length)
                   int num_packets, const uint32_t* v_list, uint32_t* bitmap,
                   uint32_t* summary, int log_src_factor, cudaStream_t stream) {
  const dim3 blkDim(BFS_PARAMS::PACKET_LENGTH / 2,
                    THREADS_PER_BLOCK * 2 / BFS_PARAMS::PACKET_LENGTH);
  assert(blkDim.x * blkDim.y == THREADS_PER_BLOCK);
  assert(blkDim.x * 2 == BFS_PARAMS::PACKET_LENGTH);
...
void fill_1_summary(uint32_t* cq_summary, int summary_size) {
