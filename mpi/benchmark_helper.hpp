...
class ProgressReport {
 public:
  ...
  void advace() {
    ...
    MPI_Isend(&send_buf_[my_progress_], 1, MPI_INT, 0, 0, MPI_COMM_WORLD,
              &send_req_[my_progress_]);
    int index, flag;
    ...
    while (complete == false) {
      ...
      while (true) {
        ...
        if (g_progress_[index] < max_progress_) {
          MPI_Irecv(&recv_buf_[index], 1, MPI_INT, index, 0, MPI_COMM_WORLD,
                    &recv_req_[index]);
        }
      }
      pthread_mutex_unlock(&thread_sync_);
...
void generate_graph(
    EdgeList* edge_list,
    const GraphGenerator<typename EdgeList::edge_type>* generator) {
  TRACER(generation);
  ...
  const int64_t num_global_chunks =
      (num_global_edges + EdgeList::CHUNK_SIZE - 1) / EdgeList::CHUNK_SIZE;
  const int64_t num_iterations =
      (num_global_chunks + mpi.size_2d - 1) / mpi.size_2d;
  ...
  if (mpi.isMaster()) {
    ...
    print_with_prefix("Graph data size: %f GB ( %f GB per process )",
                      global_data_size, local_data_size);
    print_with_prefix("Using storage: %s",
    ...
    print_with_prefix("Generating graph: Total number of iterations: %" PRId64
                      "",
                      num_iterations);
  }
#if REPORT_GEN_RPGRESS
...
#pragma omp parallel
  for (int64_t i = 0; i < num_iterations; ++i) {
    SET_OMP_AFFINITY;
    const int64_t start_edge =
        std::min((mpi.size_2d * i + mpi.rank_2d) * EdgeList::CHUNK_SIZE,
                 num_global_edges);
    const int64_t end_edge =
        std::min(start_edge + EdgeList::CHUNK_SIZE, num_global_edges);
...
#pragma omp master
    {
      ...
      if (mpi.isMaster()) {
        print_with_prefix("Time for iteration %" PRId64 " is %f ", i,
                          MPI_Wtime() - logging_time);
...
template <typename EdgeList>
void generate_graph_spec2010(EdgeList* edge_list, int scale, int edge_factor,
                             int max_weight = 0) {
  RmatGraphGenerator<typename EdgeList::edge_type, 5700, 1900> generator(
      scale, edge_factor, 255, PRM::USERSEED1, PRM::USERSEED2,
      InitialEdgeType::NONE);
  generate_graph(edge_list, &generator);
}

template <typename EdgeList>
void generate_graph_spec2012(EdgeList* edge_list, int scale, int edge_factor,
                             int max_weight) {
  RmatGraphGenerator<typename EdgeList::edge_type, 5500, 100> generator(
      scale, edge_factor, max_weight, PRM::USERSEED1, PRM::USERSEED2,
      InitialEdgeType::BINARY_TREE);
  generate_graph(edge_list, &generator);
}
...
// function #1
template <typename EdgeList>
void redistribute_edge_2d(EdgeList* edge_list,
                          typename EdgeList::edge_type::has_weight dummy = 0) {
...
template <typename EdgeList>
void redistribute_edge_2d(EdgeList* edge_list,
                          typename EdgeList::edge_type::no_weight dummy = 0) {
  typedef typename EdgeList::edge_type EdgeType;
  ScatterContext scatter(mpi.comm_2d);
  EdgeType* edges_to_send = static_cast<EdgeType*>(
      xMPI_Alloc_mem(EdgeList::CHUNK_SIZE * sizeof(EdgeType)));
...
void decode_edge(GraphType& g, int64_t e0, int64_t e1, int64_t& v0, int64_t& v1,
                 int& weight) {
  ...
  const int64_t v0_high_mask =
      ((INT64_C(1) << (log_local_verts - packing_edge_lists)) - 1);
  ...
  v0 = (((v0_high << packing_edge_lists) | v0_middle) << log_size) |
       ((rank_c << log_size_r) | v0_r);
...
int64_t find_max_used_vertex(GraphType& g) {
  ...
  MPI_Allreduce(&send_max_used_vertex, &max_used_vertex, 1, MPI_INT64_T,
                MPI_MAX, MPI_COMM_WORLD);
  return max_used_vertex;
}

int read_log_file(LogFileFormat* log, int SCALE, int edgefactor,
                  double* bfs_times, double* validate_times,
                  double* edge_counts) {
    ...
    if (mpi.isMaster()) {
      ...
      if (fp != NULL) {
        ...
        if (log->scale != SCALE || log->edge_factor != edgefactor ||
          ...
          print_with_prefix(
              "Log file is not match the current run: params:(current),(log): "
              "SCALE:%d,%d, edgefactor:%d,%d, size:%d,%d",
              SCALE, log->scale, edgefactor, log->edge_factor, mpi.size_2d,
              log->mpi_size);
          resume_root_idx = -2;
        } else {
...
void update_log_file(LogFileFormat* log, double bfs_time, double validate_time,
                     int64_t edge_counts) {
  const char* logfilename = getenv("LOGFILE");
...
void init_log(int SCALE, int edgefactor, double gen_time, double cons_time,
              double redis_time, LogFileFormat* log) {
  log->scale = SCALE;
