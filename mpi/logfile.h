...
void print_bfs_result(int num_bfs_roots, double* bfs_times,
                      double* validate_times, double* edge_counts,
                      bool result_ok) {
  if (!result_ok) {
     ...
     * Source: The Annals of Mathematical Statistics, Vol. 11, No. 4 (Dec.,
     * 1940), pp. 445-448 Publisher(s): Institute of Mathematical Statistics
     * Stable URL: http://www.jstor.org/stable/2235723
     * (same source as in specification). */
    fprintf(stdout, "harmonic_stddev_TEPS:           %.12g\n",
            stats[s_std] /
                (stats[s_mean] * stats[s_mean] * sqrt(num_bfs_roots - 1)));
    free(secs_per_edge);
    secs_per_edge = NULL;
