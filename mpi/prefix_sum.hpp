...
// itermid: offset: +1
template <typename T, int LOG_MAX>
__global__ void prefix_sum_up_phase(T* idata, T* intermid, const int num_blocks,
                                    const int num_loops) {
  const int tid = threadIdx.x;
...
// itermid: offset: +0
template <typename T, int LOG_MAX>
__global__ void prefix_sum_down_phase(T* idata, T* intermid,
...
__device__ void dev_prefix_sum(T* s_data, const int tid) {
  ...
  if (LOG_MAX >= 11) {
    if (tid < 1024) {
      s_data[OFFSET_2048 + tid] = s_data[tid2 + 0] + s_data[tid2 + 1];
    }
    __syncthreads();
  }
  if (LOG_MAX >= 10) {
    if (tid < 512) {
      s_data[OFFSET_1024 + tid] =
          s_data[OFFSET_2048 + tid2 + 0] + s_data[OFFSET_2048 + tid2 + 1];
    }
    __syncthreads();
  }
  if (LOG_MAX >= 9) {
    if (tid < 256) {
      s_data[OFFSET_512 + tid] =
          s_data[OFFSET_1024 + tid2 + 0] + s_data[OFFSET_1024 + tid2 + 1];
    }
    __syncthreads();
  }
  if (LOG_MAX >= 8) {
    if (tid < 128) {
      s_data[OFFSET_256 + tid] =
          s_data[OFFSET_512 + tid2 + 0] + s_data[OFFSET_512 + tid2 + 1];
    }
    __syncthreads();
  }
  if (LOG_MAX >= 7) {
    if (tid < 64) {
      s_data[OFFSET_128 + tid] =
          s_data[OFFSET_256 + tid2 + 0] + s_data[OFFSET_256 + tid2 + 1];
    }
    __syncthreads();
  }
  if (LOG_MAX >= 6) {
    if (tid < 32) {
      s_data[OFFSET_64 + tid] =
          s_data[OFFSET_128 + tid2 + 0] + s_data[OFFSET_128 + tid2 + 1];
    }
    __syncthreads();
  }

  if (tid < 32) {
    dev_simd_kogge_stone<T, 5>(s_data + OFFSET_64, tid);
  }
  __syncthreads();
  ...
  if (tid < HARF_SIZE) {
    ...
    if (init_v1 != diff_v1) {
      printf("KU:%d:R(%d)B(%d)T(%d,%d):PrefixSumError1(%d!=%d)\n", __LINE__,
             mpig.rank_2d, blockIdx.x, threadIdx.x, threadIdx.y, init_v1,
             diff_v1);
    }
    if (init_v2 != diff_v2) {
      printf("KU:%d:R(%d)B(%d)T(%d,%d):PrefixSumError2(%d!=%d)\n", __LINE__,
             mpig.rank_2d, blockIdx.x, threadIdx.x, threadIdx.y, init_v2,
             diff_v2);
    }
    if (tid == 0 && base_v != s_data[0]) {
      printf("KU:%d:R(%d)B(%d)T(%d,%d):PrefixSumError3(%d!=%d)\n", __LINE__,
             mpig.rank_2d, blockIdx.x, threadIdx.x, threadIdx.y, base_v,
             s_data[0]);
    }
  }
#endif
...
class vector2 {
 public:
  ...
  __host__ __device__ vector2(T x__) : x(x__), y(x__) {}
  __host__ __device__ vector2(T x__, T y__) : x(x__), y(y__) {}
  __host__ __device__ vector2(const vector2<T>& r) : x(r.x), y(r.y) {}
  __host__ __device__ vector2(const volatile vector2<T>& r) : x(r.x), y(r.y) {}
  ...
  __host__ __device__ volatile vector2<T>& operator+=(
      const vector2<T> r) volatile {
    x += r.x;
    y += r.y;
    return *this;
  }
...
PrefixSumGPU<T>::PrefixSumGPU(int64_t max_length, int num_partitions) {
  ...
  CUDA_CHECK(cudaMalloc(
      (void**)&dev_array_,
      sizeof(T) * num_partitions_ * (max_length_ + PADDING + MAX_ACTIVE_CTAS)));
}
template <typename T>
...
void PrefixSumGPU<T>::operator()(cudaStream_t* streams, int num_streams) {
  ...
  if (log_max_length_ <= THRESHOLD) {  // 4K
    ...
    for (int i = 0; i < num_partitions_; ++i)
      prefix_sum_up_phase<T, LOG_MAX><<<num_ctas, CTA_DIM_X / 2, 0, STREAM_I>>>(
          ARRAY_I + 1, INTERMID_I, num_blocks, num_loops);
    for (int i = 0; i < num_partitions_; ++i)
      prefix_sum_one_block<T, LOG_MAX_ACTIVE_CTAS>
          <<<1, MAX_ACTIVE_CTAS / 2, 0, STREAM_I>>>(INTERMID_I);
    for (int i = 0; i < num_partitions_; ++i)
      prefix_sum_down_phase<T, LOG_MAX><<<num_ctas, CTA_DIM_X, 0, STREAM_I>>>(
          ARRAY_I + 1, INTERMID_I - 1, num_blocks, num_loops);
  }
}
